...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!hasPermissions) {
    await knex.schema.createTable('permissions', (table) => {
      ...
      table.string('grantee', 255).notNullable(); // account address
      ...
      table.string('validator_perm_id', 50).nullable(); // Can be null for ECOSYSTEM permissions
  ...
  // Create permission_events table for audit trail if not exists
  const hasEvents = await knex.schema.hasTable('permission_events');
  if (!hasEvents) {
    await knex.schema.createTable('permission_events', (table) => {
      table.bigIncrements('id').primary();
      table.bigInteger('perm_id').unsigned().notNullable();
      table.string('action', 50).notNullable(); // CREATE, UPDATE, VALIDATE, REVOKE, etc.
      table.string('actor', 255).notNullable(); // account address
      table.jsonb('data').nullable(); // Event-specific data
      table.timestamp('created').defaultTo(knex.fn.now());
