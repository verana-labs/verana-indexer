...
export default class CreateTableEventPartitionSpec {
  ...
  @BeforeEach()
  async initSuite() {
    this.createEventPartitionJob = this.broker.createService(CreateEventPartitionJob) as CreateEventPartitionJob
  }

  @Test('No event exist on table => Dont need to create partition')
  public async test1() {
    await knex.raw('TRUNCATE TABLE event RESTART IDENTITY CASCADE')
    const result = await this.createEventPartitionJob?.jobCreateEventPartition()
  ...
  public async test2() {
    ...
    const result = await this.createEventPartitionJob?.createPartitionName(mockEvent)
    ...
    const result1 = this.createEventPartitionJob?.createPartitionName(mockEvent1)
  ...
  public async test3() {
    ...
    const partitionInfo = await this.createEventPartitionJob?.createPartitionName(mockEvent)
    ...
    const partitionInfo1 = await this.createEventPartitionJob?.createPartitionName(mockEvent1)
    expect(partitionInfo1?.fromEventId).toEqual('1000000000')
    expect(partitionInfo1?.toEventId).toEqual('1200000000')
    expect(partitionInfo1?.partitionName).toEqual('event_partition_1000000000_1200000000')
  }
  ...
  public async test4() {
    ...
    const partitionInfo = await this.createEventPartitionJob?.createPartitionName(mockEvent)
    if (partitionInfo) {
      await this.createEventPartitionJob?.createPartitionByPartitionInfo(partitionInfo)
    }

    /**
     * @description partition now created so isCreated now will be false because partition already exist
     */
    const checkAgainPartitionInfo = await this.createEventPartitionJob?.createPartitionName(mockEvent)
