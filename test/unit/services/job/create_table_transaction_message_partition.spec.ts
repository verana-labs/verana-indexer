...
export default class CreateTableTransactionMessagePartitionSpec {
  ...
  @Test('No transaction message exist on table => Dont need to create partition')
  public async test1() {
    await knex.raw('TRUNCATE TABLE transaction_message RESTART IDENTITY CASCADE')
    const result = await this.createTxMsgPartitionJob?.jobCreateTransactionMessagePartition()
  ...
  public async test2() {
    ...
    const result = await this.createTxMsgPartitionJob?.createPartitionName(mockTxMsg)
    ...
    const result1 = this.createTxMsgPartitionJob?.createPartitionName(mockTxMsg1)
  ...
  public async test3() {
    ...
    const mockTxMsg = new TransactionMessage()
    mockTxMsg.id = config.migrationTransactionMessageToPartition.step / 2 + 1
    const partitionInfo = await this.createTxMsgPartitionJob?.createPartitionName(mockTxMsg)
    ...
    const partitionInfo1 = await this.createTxMsgPartitionJob?.createPartitionName(mockTxMsg1)
    ...
    expect(partitionInfo1?.partitionName).toEqual('transaction_message_partition_900000000_1000000000')
  }
