...
export default class CreateTableTransactionPartitionSpec {
  ...
  public async test1() {
    ...
    const result = await this.createTransactionPartitionJob?.jobCreateTransactionPartition()
  ...
  public async test2() {
    ...
    const result = await this.createTransactionPartitionJob?.createPartitionName(mockTransaction)
    ...
    const result1 = this.createTransactionPartitionJob?.createPartitionName(mockTransaction1)
  ...
  public async test3() {
    ...
    const mockTransaction = new Transaction()
    mockTransaction.id = config.migrationTransactionToPartition.step / 2 + 1
    const partitionInfo = await this.createTransactionPartitionJob?.createPartitionName(mockTransaction)
    ...
    const partitionInfo1 = await this.createTransactionPartitionJob?.createPartitionName(mockTransaction1)
    expect(partitionInfo1?.fromTransactionId).toEqual('900000000')
    expect(partitionInfo1?.toTransactionId).toEqual('1000000000')
    expect(partitionInfo1?.partitionName).toEqual('transaction_partition_900000000_1000000000')
  }
