...
export default class CreateTransactionMessageConstraintPartitionSpec {
  ...
  public async test1() {
    await knex.raw('TRUNCATE TABLE transaction_message RESTART IDENTITY CASCADE')
    ...
    const emptyStatus = await this.createConstaintTxMsgJob?.createTransactionMessageConstraint(partitions2[0])
    expect(emptyStatus).toEqual(this.createConstaintTxMsgJob?.createConstraintTxMsgStatus.currentPartitionEmpty)
    ...
    const constraintUpdated = await this.createConstaintTxMsgJob?.createTransactionMessageConstraint(partitions2[0])
    // When partition is inserting and no constraint exists, it should create constraint and return constraintUpdated
    expect(constraintUpdated).toEqual(this.createConstaintTxMsgJob?.createConstraintTxMsgStatus.constraintUpdated)
    ...
    const stillInsertingOrDont = await this.createConstaintTxMsgJob?.createTransactionMessageConstraint(partitions2[0])
    expect(stillInsertingOrDont).toEqual(
      this.createConstaintTxMsgJob?.createConstraintTxMsgStatus.currentPartitionDoneOrInserting
    )
    ...
    const constraintCreatedDone = await this.createConstaintTxMsgJob?.createTransactionMessageConstraint(partitions2[0])
    expect(constraintCreatedDone).toEqual(this.createConstaintTxMsgJob?.createConstraintTxMsgStatus.constraintUpdated)
    ...
    const isDoneConstraintExist = await this.isConstraintNameExist(partitions2[0].name, expectedDoneConstraintName)
    ...
    const checkAgainStatus = await this.createConstaintTxMsgJob?.createTransactionMessageConstraint(partitions2[0])
