...
export default class CreateEventConstraintPartitionSpec {
  ...
  private async isConstraintNameExist(partitionName: string, constraintName: string): Promise<boolean> {
  ...
  public async test1() {
    await knex.raw('TRUNCATE TABLE block, transaction, event, event_partition RESTART IDENTITY CASCADE')
    await knex.raw('ALTER TABLE event RENAME TO event_backup')
    await knex.raw('ALTER TABLE event_partition RENAME TO event')
    await knex.raw('CREATE TABLE event_partition_0_200000000 (LIKE event INCLUDING ALL)')
    await knex.raw('ALTER TABLE event ATTACH PARTITION event_partition_0_200000000 FOR VALUES FROM (0) TO (200000000)')
    const partitions = await this.createConstraintInEventPartitionJob?.getEventPartitionInfo()
    ...
    const emptyStatus = await this.createConstraintInEventPartitionJob?.createEventConstraint(partitions[0])
    expect(emptyStatus).toEqual(
      this.createConstraintInEventPartitionJob?.createConstraintEventStatus.currentPartitionEmpty
    )
    ...
    const constraintUpdated = await this.createConstraintInEventPartitionJob?.createEventConstraint(partitions[0])
    expect(constraintUpdated).toEqual(
      this.createConstraintInEventPartitionJob?.createConstraintEventStatus.constraintUpdated
    )
    ...
    const stillInsertingOrDont = await this.createConstraintInEventPartitionJob?.createEventConstraint(partitions[0])
    expect(stillInsertingOrDont).toEqual(
      this.createConstraintInEventPartitionJob?.createConstraintEventStatus.currentPartitionDoneOrInserting
    )
    ...
    const constraintCreatedDone = await this.createConstraintInEventPartitionJob?.createEventConstraint(partitions[0])
    ...
    const isDoneConstraintExist = await this.isConstraintNameExist(partitions[0].name, expectedDoneConstraintName)
    ...
    const checkAgainStatus = await this.createConstraintInEventPartitionJob?.createEventConstraint(partitions[0])
