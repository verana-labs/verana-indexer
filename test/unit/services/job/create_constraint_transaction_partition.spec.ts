...
export default class CreateConstraintTransactionPartitionSpec {
  ...
  public async test1() {
    ...
    const partitions = await this.createConstraintInTransactionPartitionJob?.getTransactionPartitionInfo()
    ...
    const emptyStatus = await this.createConstraintInTransactionPartitionJob?.createTransactionConstraint(partitions[0])
    expect(emptyStatus).toEqual(
      this.createConstraintInTransactionPartitionJob?.createConstraintTransactionStatus.currentPartitionEmpty
    )

    // After insert one tx, now we expect constraint created
    await this.insertFakeTxWithInputId(Number(partitions[0].fromId) + 1)
    const constraintUpdated = await this.createConstraintInTransactionPartitionJob?.createTransactionConstraint(
      partitions[0]
    )
    expect(constraintUpdated).toEqual(
      this.createConstraintInTransactionPartitionJob?.createConstraintTransactionStatus.constraintUpdated
    )
    ...
    const stillInsertingOrDont = await this.createConstraintInTransactionPartitionJob?.createTransactionConstraint(
      partitions[0]
    )
    expect(stillInsertingOrDont).toEqual(
      this.createConstraintInTransactionPartitionJob?.createConstraintTransactionStatus.currentPartitionDoneOrInserting
    )
    ...
    const constraintCreatedDone = await this.createConstraintInTransactionPartitionJob?.createTransactionConstraint(
      partitions[0]
    )
    ...
    const checkAgainStatus = await this.createConstraintInTransactionPartitionJob?.createTransactionConstraint(
      partitions[0]
    )
