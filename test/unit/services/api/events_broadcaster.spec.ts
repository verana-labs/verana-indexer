...
function broadcastBlockIndexed(b: SubscribeBroadcaster, height: number, ts: Date | string): void {
...
async function openSubscribed(url: string, dids: string[] | null): Promise<WebSocket> {
...
describe('SubscribeBroadcaster', () => {
  ...
  it('accepts WebSocket connections and sends a ready message', async () => {
    ...
    expect(Object.hasOwn(message, 'block')).toBe(true)
    expect(Object.hasOwn(message, 'blockTime')).toBe(true)
  ...
  it('delivers persisted indexer events only to matching DID subscribers', async () => {
    ...
    const matchPromise = waitForMessage(wsMatch, (msg) => msg.type === 'block' && msg.events.length > 0)
