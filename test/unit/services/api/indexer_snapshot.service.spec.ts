...
describe('IndexerSnapshotService snapshot endpoint', () => {
  ...
  const _didB = `did:web:snapshot-b-${runId}.example`
  ...
  async function _insertRow(table: string, row: Record<string, any>): Promise<void> {
    const info = await getColumnInfo(table)
    const filtered: Record<string, any> = {}
    for (const [k, v] of Object.entries(row)) {
      if (Object.hasOwn(info, k)) filtered[k] = v
    }
