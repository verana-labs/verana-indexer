...
import { BlockCheckpoint, Delegator, Transaction, TransactionMessage, Validator } from '../../../../src/models'
...
export default class CrawlDelegatorsTest {
  ...
  async initSuite() {
    ...
    this.crawlDelegatorsService = this.broker.createService(CrawlDelegatorsService) as CrawlDelegatorsService

    try {
      this.crawlDelegatorsService.getQueueManager().stopAll()
    } catch {}

    await Promise.all([Validator.query().insert(this.validator), BlockCheckpoint.query().insert(this.blockCheckpoint)])
  }
  ...
  async tearDown() {
    ...
    await Promise.all([
      ...
      knex.raw('TRUNCATE TABLE transaction, transaction_message, block_checkpoint RESTART IDENTITY CASCADE'),
    ])
  ...
  private async insertCrawlDelegatorDependingJob(desiredTxId: number, desiredHeight: number): Promise<void> {
  ...
  public async test1(): Promise<void> {
    ...
    const validator = await Validator.query().findOne('operator_address', this.validator.operator_address)
  ...
  public async test7(): Promise<void> {
    ...
    const validatorSrc = await Validator.query().findOne('operator_address', this.validator.operator_address)
    const validatorDst = await Validator.query().findOne('operator_address', newValidator.operator_address)
  ...
  public async test8(): Promise<void> {
    ...
    await Validator.query().insert(this.validator)
    await this.insertCrawlDelegatorDependingJob(mockCrawlValidatorTxId, mockCrawlValidatorHeight)
    await this.insertFakeTxMsg(
      MSG_TYPE.MSG_DELEGATE,
      this.mockDelegatorAddress,
      mockDelegateAmount,
    ...
    await this.insertCrawlDelegatorDependingJob(mockCrawlValidatorTxId + 1, mockCrawlValidatorHeight + 1)
    await this.crawlDelegatorsService.handleJob()
    ...
    const testAgainValidator = await Validator.query().findOne('operator_address', this.validator.operator_address)
