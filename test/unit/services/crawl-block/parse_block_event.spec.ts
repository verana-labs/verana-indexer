...
export default class CrawlBlockTest {
  ...
  blocks = [
    {
      ...
      block_result: {
        ...
        begin_block_events: [
          ...
          { type: 'message', attributes: [{ key: 'c2VuZGVy', value: 'dmVyYW5hMWFkZHIx', index: true }] },
          {
            type: 'mint',
            attributes: [
          ...
          { type: 'message', attributes: [{ key: 'c2VuZGVy', value: 'dmVyYW5hMWFkZHIy', index: true }] },
          {
        ...
        end_block_events: [
          ...
          { type: 'message', attributes: [{ key: 'c2VuZGVy', value: 'dmVyYW5hMWFkZHI0', index: true }] },
        ],
        validator_updates: [],
  ...
  async initSuite() {
    ...
    const stopQueues = async (svc: any) => {
      try {
        if (svc?.getQueueManager?.()) await svc.getQueueManager().stopAll()
      } catch {}
    }
  ...
  public async testHandleBlocks() {
    ...
    const rows = await knex('block').select('*').where('height', 2001002).orderBy('height', 'asc')
