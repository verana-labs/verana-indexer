...
export default class CrawlIbcTest {
  ...
  crawlIbcAppSerivce = this.broker.createService(CrawlIbcAppService) as CrawlIbcAppService
  ...
  async initSuite() {
    ...
    await knex.raw(
      'TRUNCATE TABLE block, block_signature, transaction, transaction_message, event, event_attribute, ibc_message RESTART IDENTITY CASCADE'
    )
  ...
  async testHandleIbcSendPacket() {
    await knex.transaction(async (trx) => {
      ...
      const newPacket = await IbcMessage.query().transacting(trx).first().throwIfNotFound()
    ...
    await knex.transaction(async (trx) => {
      const content = {
        ...
        packet: {
          ...
          sequence: '1',
          source_port: 'wasm.nois1s9ly26evj8ehurptws5d6dm4a9g2z0htcqvlvn95kc30eucl4s5sd8hkgp',
          source_channel: 'channel-24',
          destination_port: 'wasm.aura1qrf8f9kyh4zzckz2zy52z5gppwweumvrlxqrgd4xr3ydf3sx4dlqt8lnt8',
      ...
      const attributes = [
        ...
        },
        {
          event_id: 1,
          key: 'packet_src_port',
          value: 'wasm.nois1s9ly26evj8ehurptws5d6dm4a9g2z0htcqvlvn95kc30eucl4s5sd8hkgp',
        },
        {
          event_id: 1,
          key: 'packet_src_channel',
        ...
        },
        {
          event_id: 1,
          key: 'packet_dst_port',
          value: 'wasm.aura1qrf8f9kyh4zzckz2zy52z5gppwweumvrlxqrgd4xr3ydf3sx4dlqt8lnt8',
        },
        {
          event_id: 1,
          key: 'packet_dst_channel',
  ...
  async testHandleIbcAckPacket() {
    await knex.transaction(async (trx) => {
      const content = {
        ...
        acknowledgement: {
          error: 'ABCI code: 1: error handling packet on destination chain: see events for details',
        },
      }
      const attributes = [
        {
