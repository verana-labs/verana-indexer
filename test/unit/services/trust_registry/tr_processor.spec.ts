...
import TrustRegistryMessageProcessorService from "../../../../src/services/trust_registry/tr_processor.service";
...
describe("TrustRegistryMessageProcessorService Tests", () => {
    ...
    beforeEach(async () => {
        await knex("governance_framework_document_history").del().catch(() => {});
        await knex("governance_framework_version_history").del().catch(() => {});
        await knex("trust_registry_history").del().catch(() => {});
    ...
    it("should insert a Trust Registry with related Governance Framework and history", async () => {
        ...
        await service.handleTrustRegistryMessages({ params: { trustRegistryList: dummyTR } } as any);
        ...
        const trHistory = await knex("trust_registry_history").where({ tr_id: tr.id }).first();
        expect(trHistory).toBeDefined();
        expect(trHistory.changes).toBeDefined(); 

        const gfvHistory = await knex("governance_framework_version_history").where({ tr_id: tr.id }).first();
        expect(gfvHistory).toBeDefined();
        expect(gfvHistory.changes).toBeDefined();

        const gfdHistory = await knex("governance_framework_document_history").where({ gfv_id: gfv.id }).first();
    ...
    it("should update an existing Trust Registry and record history", async () => {
        ...
        await service.handleTrustRegistryMessages({ params: { trustRegistryList: UpdateResponse } } as any);
        ...
        const trHistory = await knex("trust_registry_history").where({ tr_id: trId.id }).orderBy("id", "desc").first();
        expect(trHistory).toBeDefined();
        const changes = typeof trHistory.changes === "string" ? JSON.parse(trHistory.changes) : trHistory.changes;
    ...
    it("should archive and unarchive a Trust Registry with history", async () => {
        ...
        await service.handleTrustRegistryMessages({ params: { trustRegistryList: archiveTrustRegistry } } as any);

        const archivedTR = await knex("trust_registry").where({ id: trId.id }).first();
        expect(archivedTR.archived).not.toBeNull();

        const trHistoryArchive = await knex("trust_registry_history").where({ tr_id: trId.id }).orderBy("id", "desc").first();
        expect(trHistoryArchive).toBeDefined();
        const archiveChanges = typeof trHistoryArchive.changes === "string" ? JSON.parse(trHistoryArchive.changes) : trHistoryArchive.changes;
        ...
        await service.handleTrustRegistryMessages({ params: { trustRegistryList: unarchiveTrustRegistry } } as any);

        const unarchivedTR = await knex("trust_registry").where({ id: trId.id }).first();
        expect(unarchivedTR.archived).toBeNull();

        const trHistoryUnarchive = await knex("trust_registry_history").where({ tr_id: trId.id }).orderBy("id", "desc").first();
        expect(trHistoryUnarchive).toBeDefined();
        const unarchiveChanges = typeof trHistoryUnarchive.changes === "string" ? JSON.parse(trHistoryUnarchive.changes) : trHistoryUnarchive.changes;
