...
describe('EcosystemMessageProcessorService Tests', () => {
  ...
  it('should insert a Trust Registry with related Governance Framework and history', async () => {
    ...
    const ec = await knex('ecosystem').where({ did: 'did:example:insert' }).first()
    expect(ec).toBeDefined()
    expect(Number(ec.corporation_id)).toBe(1)

    const gfv = await knex('governance_framework_version').where({ ecosystem_id: ec.id }).first()
    ...
    const ecosystemHistory = await knex('ecosystem_history').where({ ecosystem_id: ec.id }).first()
    expect(ecosystemHistory).toBeDefined()
    expect(ecosystemHistory.changes).toBeDefined()

    const gfvHistory = await knex('governance_framework_version_history').where({ ecosystem_id: ec.id }).first()
  ...
  it('should update an existing Trust Registry and record history', async () => {
    ...
    const updatedTR = await knex('ecosystem').where({ id: ecosystemId.id }).first()
    ...
    expect(ecosystemHistory).toBeDefined()
    const changes =
      typeof ecosystemHistory.changes === 'string' ? JSON.parse(ecosystemHistory.changes) : ecosystemHistory.changes
  ...
  it('should archive and unarchive a Trust Registry with history', async () => {
    ...
    const archivedTR = await knex('ecosystem').where({ id: ecosystemId.id }).first()
    ...
    const unarchivedTR = await knex('ecosystem').where({ id: ecosystemId.id }).first()
  ...
  it('should create multiple Trust Registries with the same DID at different heights', async () => {
    ...
    const firstTRRecord = await knex('ecosystem').where({ height: height1 }).first()
    expect(firstTRRecord).toBeDefined()
    expect(firstTRRecord.did).toBe(sameDid)
    expect(firstTRRecord.height).toBe(height1)

    const firstGFV = await knex('governance_framework_version').where({ ecosystem_id: firstTRRecord.id }).first()
    expect(firstGFV).toBeDefined()

    const firstGFD = await knex('governance_framework_document').where({ gfv_id: firstGFV.id }).first()
    ...
    const secondTRRecord = await knex('ecosystem').where({ height: height2 }).first()
    ...
    const firstTRRecordAfter = await knex('ecosystem').where({ id: firstTRRecord.id }).first()
    ...
    const secondGFV = await knex('governance_framework_version').where({ ecosystem_id: secondTRRecord.id }).first()
    expect(secondGFV).toBeDefined()
    expect(secondGFV.id).not.toBe(firstGFV.id) // Should be a different GFV

    const secondGFD = await knex('governance_framework_document').where({ gfv_id: secondGFV.id }).first()
    ...
    const firstGFDAfter = await knex('governance_framework_document').where({ gfv_id: firstGFV.id }).first()
