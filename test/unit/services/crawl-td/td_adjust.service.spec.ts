...
jest.mock('../../../../src/models/block_checkpoint', () => {
  ...
  return {
    BlockCheckpoint: {
      ...
      getCheckpoint: jest.fn().mockResolvedValue([0, 0, { id: 'cp1', job_name: 'crawl:trust-deposit', height: 0 }]),
    },
  }
})
...
describe('🧪 TrustDepositDatabaseService', () => {
  ...
  const _service = broker.createService(TrustDepositDatabaseService)
  ...
  it('✅ should process transactions with adjust_trust_deposit event', async () => {
    ...
    const result = await broker.call(SERVICE.V1.TrustDepositDatabaseService.path + '.adjustTrustDeposit', {
      account: 'verana1evvrzxw9yg5staqdvumd6fupy3jhaxfflla7st',
