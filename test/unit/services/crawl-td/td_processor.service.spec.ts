...
describe('🧪 TrustDepositMessageProcessorService', () => {
  ...
  it('✅ should process empty message list gracefully', async () => {
    const res = await broker.call(SERVICE.V1.TrustDepositMessageProcessorService.path + '.handleTrustDepositMessages', {
      trustDepositList: [],
    })
  ...
  it('✅ should process unknown message type', async () => {
    const warnSpy = jest
      // @ts-expect-error - logger injected by moleculer
      .spyOn((processorService as any).logger, 'warn')
      .mockImplementation(() => undefined as unknown as any)
    await broker.call(SERVICE.V1.TrustDepositMessageProcessorService.path + '.handleTrustDepositMessages', {
      trustDepositList: [
        {
          type: 'UNKNOWN',
          content: {
  ...
  it('✅ should slash trust deposit successfully', async () => {
    ...
    const res = await broker.call(SERVICE.V1.TrustDepositDatabaseService.path + '.slash_trust_deposit', {
      account: 'verana1evvrzxw9yg5staqdvumd6fupy3jhaxfflla7st',
      slashed: '100',
  ...
  it('❌ should throw error if slash called with invalid params', async () => {
    await expect(
      broker.call(SERVICE.V1.TrustDepositDatabaseService.path + '.slash_trust_deposit', {
        slashed: '100',
      })
