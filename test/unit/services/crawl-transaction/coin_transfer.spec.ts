...
import { Block, BlockCheckpoint, CoinTransfer, Transaction } from '../../../../src/models'
...
export default class CoinTransferSpec {
  ...
  @BeforeEach()
  async initSuite() {
    this.coinTransferService = this.broker.createService(CoinTransferService) as CoinTransferService
  ...
  public async testSingleCoinTransfer() {
    ...
    // Validate
    const coinTransfer = await CoinTransfer.query().where('block_height', txHeight)
  ...
  public async testMultiCoinTransfer() {
    ...
    // Validate
    const coinTransfers = await CoinTransfer.query().where('block_height', txHeight)
  ...
  public async testAuthzCoinTransfer() {
    const providerRegistry = await getProviderRegistry()
    const chainRegistry = new ChainRegistry(this.broker.logger, providerRegistry)
