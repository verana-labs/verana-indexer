...
import { Block, BlockCheckpoint, Transaction, TransactionMessage } from '../../../../src/models'
...
export default class HandleAuthzTxMsgTest {
  ...
  @BeforeAll()
  async initSuite() {
    this.handleAuthzTxServive = this.broker.createService(HandleAuthzTxService) as HandleAuthzTxService
  ...
  public async testHandleTransactionAuthz() {
    ...
    if (tx) {
      const txMessages = await TransactionMessage.query().joinRelated('transaction').where('transaction.id', tx.id)
