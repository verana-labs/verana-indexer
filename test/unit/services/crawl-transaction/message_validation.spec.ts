...
export default class MessageValidationTest {
  ...
  public testKnownMessageTypes() {
    const knownTypes = new Set([...ALL_KNOWN_VERANA_MESSAGE_TYPES])
  ...
  public testUnknownMessageTypes() {
    const knownTypes = new Set(['/verana.cs.v1.MsgCreateCredentialSchema', '/verana.cs.v1.MsgUpdateCredentialSchema'])
  ...
  public testNonVeranaMessagesIgnored() {
    const knownTypes = new Set(['/verana.cs.v1.MsgCreateCredentialSchema'])
  ...
  public testCrashSimulation() {
    ...
    try {
      const knownVeranaMessageTypes = new Set([...ALL_KNOWN_VERANA_MESSAGE_TYPES])
    ...
    try {
      ...
      if (unknownMessages.length > 0) {
        ...
        expect(mockConsoleWarn).toHaveBeenCalledWith(
          'TEST MODE: Skipping validation for unknown Verana message types: /verana.unknown.v1.MsgUnknownType'
        )
      }
    } finally {
      mockExit.mockRestore()
