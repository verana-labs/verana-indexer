...
async function makeRequest(method: string, url: string, config: any = {}): Promise<AxiosResponse> {
...
const _isCI = String(process.env.CI || '').toLowerCase() === 'true'
...
describeIf('Comprehensive API Endpoints Integration Tests', () => {
  ...
  describe('Indexer Endpoints - All Parameters Tested', () => {
    ...
    itIf('should list VT changes - valid fromBlock with a channel', async () => {
      const response = await testEndpoint(
        'GET',
        `/v4/verifiable-trust/changes?fromBlock=${SAMPLE_BLOCK_HEIGHT}&channels=trust`
      )
      expect(response.status).not.toBeGreaterThanOrEqual(500)
      if (response.status === 200) {
