...
import { extractImpactedCredentialSchemaIds, getCredentialSchema } from './cs_height_sync_helpers'
...
function getCsHeightSyncLogger(broker: ServiceBroker): CsHeightSyncLogger {
  ...
  return brokerWithLogger.getLogger?.('cs-height-sync') ?? brokerWithLogger.logger ?? console
}
...
export async function syncCredentialSchemas(
  ...
  const ids = [...new Set(credentialSchemaIds)].filter((id): id is number => Number.isInteger(id) && id > 0)
  ...
  const outcomes = await runWithConcurrency(ids, CS_CONCURRENCY, async (id) => {
    try {
      ...
      if (!ledgerResponse?.schema) {
        logger.warn?.(`[CS Height Sync] Ledger returned no schema for id=${id} at block=${blockHeightNum}`)
        return false
      }
      const result = (await broker.call(`${SERVICE.V1.CredentialSchemaDatabaseService.path}.syncFromLedger`, {
...
export async function runHeightSyncCS(
  ...
  const logger = getCsHeightSyncLogger(broker)
  const hasActivity =
    (payload.credentialSchemaMessages?.length ?? 0) > 0 || (payload.csEventsFromBlock?.length ?? 0) > 0
  ...
  logger.info?.(`[CS Height Sync] Starting sync at block=${blockHeight} for ${ids.length} schema(s): ${ids.join(',')}`)
  ...
  try {
    ...
    logger.error?.(`[CS Height Sync] Failed sync at block=${blockHeight}: ${formatCsHeightSyncError(error)}`)
  }
}
