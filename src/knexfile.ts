...
const toInt = (value: string | number | undefined, fallback: number): number => parseInt(String(value ?? fallback), 10)
...
const createPool = (min: number, max: number, acquireTimeout: number = 60000) => {
  ...
  return {
    ...
    afterCreate: (conn: any, done: (err: Error | null, conn: any) => void) => {
      conn.query(`SET idle_in_transaction_session_timeout = ${idleInTxTimeout};`, (err: Error | null) =>
        done(err, conn)
      )
    },
  }
...
export const knexConfig: Record<string, Knex.Config> = {
  ...
  test: {
    ...
    pool: isLightweightMode() ? createPool(1, 2, lightweightAcquireTimeout) : createPool(1, 5, 60000),
    acquireConnectionTimeout: isLightweightMode() ? lightweightAcquireTimeout : 60000,
    asyncStackTraces: false,
  },
  production: {
