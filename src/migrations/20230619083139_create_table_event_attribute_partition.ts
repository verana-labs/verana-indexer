...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.transaction(async (trx) => {
    ...
    await knex.raw('alter table event_attribute rename to event_attribute_backup;').transacting(trx)
    await knex.raw('alter table event_attribute_partition rename to event_attribute;').transacting(trx)
    const currentId = {
      event_id: 0,
      index: 0,
    }
    ...
    for (let i = startBlock; i < endBlock; i += step) {
      ...
      await knex.raw(`create table ${tableName} (like event_attribute including all)`).transacting(trx)
      // attach partition to table event_attribute
      await knex
        .raw(`alter table event_attribute attach partition ${tableName} for values from (${i}) to (${i + step})`)
        .transacting(trx)
    }
    ...
    const limitRecordGet = config.migrationEventAttributeToPartition.limitRecordGet
    const chunkSizeInsert = config.migrationEventAttributeToPartition.chunkSizeInsert
    let done = false
    while (!done) {
      ...
      const eventAttributes = await knex('event_attribute_backup')
        .select('*')
        .whereRaw(`(event_id, index) > (${currentId.event_id}, ${currentId.index})`)
        .orderBy([
      ...
      await knex.batchInsert('event_attribute', eventAttributes, chunkSizeInsert).transacting(trx)
    ...
    await knex.schema.createViewOrReplace('view_event_attribute_value_index', (viewBuilder) => {
      viewBuilder.as(knex('event_attribute').select('*').whereRaw('length(value) <= 100'))
    })
  })
}
...
export async function down(knex: Knex): Promise<void> {
  await knex.transaction(async (trx) => {
    await knex.raw('alter table event_attribute rename to event_attribute_revert;').transacting(trx)
    await knex.raw('alter table event_attribute_backup rename to event_attribute;').transacting(trx)
