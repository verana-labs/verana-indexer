...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.transaction(async (trx) => {
    ...
    const ibcMsgs = await IbcMessage.query()
      ...
      .select('message:transaction.timestamp', 'ibc_message.sequence_key', 'ibc_message.type')
    if (ibcIcs20s.length > 0) {
      ibcIcs20s.forEach((ibcIcs20) => {
        ibcIcs20.start_time =
          ibcIcs20.type === IbcMessage.EVENT_TYPE.SEND_PACKET
            ? ibcMsgs.find(
                (e) => e.sequence_key === ibcIcs20.sequence_key && e.type === IbcMessage.EVENT_TYPE.SEND_PACKET
              )?.timestamp
            : null
        ibcIcs20.finish_time = ibcMsgs.find(
          (e) =>
            ...
            (ibcIcs20.type === IbcMessage.EVENT_TYPE.SEND_PACKET
              ? [IbcMessage.EVENT_TYPE.TIMEOUT_PACKET, IbcMessage.EVENT_TYPE.ACKNOWLEDGE_PACKET].includes(e.type)
              : [IbcMessage.EVENT_TYPE.RECV_PACKET].includes(e.type))
        )?.timestamp
      })
      await IbcIcs20.query().transacting(trx).insert(ibcIcs20s).onConflict('id').merge()
    }
  })
}
