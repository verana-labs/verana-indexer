...
async function renameColumns(knex: Knex, triples: Array<[string, string, string]>): Promise<void> {
...
const INDEX_RENAME_SQL = (forward: boolean) => {
  ...
  const replaces = subs.map(([a, b]) => `newname := replace(newname, ${quote(a)}, ${quote(b)});`).join('\n      ')
...
async function migrateModuleValues(knex: Knex, pairs: Array<[string, string]>): Promise<void> {
...
export async function down(knex: Knex): Promise<void> {
  ...
  await renameColumns(
    ...
    COLUMN_RENAMES.map(
      ([table, from, to]) =>
        [TABLE_RENAMES.find(([o]) => o === table)?.[1] ?? table, to, from] as [string, string, string]
    )
  )
  await renameTables(
    knex,
