...
const isPostgres = (knex: Knex): boolean => String(knex.client.config?.client || '').includes('pg')
...
const PERM_SESSION_HISTORY_SESSION_HEIGHT_MODIFIED_IDX =
  'idx_permission_session_history_session_height_modified_created_id_desc'
...
const PH_SCHEMA_EVENT_HEIGHT_PERMISSION_CREATED_ID_IDX =
  'idx_permission_history_schema_event_height_permission_created_id_desc'
...
export async function up(knex: Knex): Promise<void> {
  ...
  const liveCorporationQueryColumns = [
    'corporation',
    'schema_id',
    'type',
    'vp_state',
  ...
  const activeListFiltersColumns = [
    'schema_id',
    'type',
    'vp_state',
    'validator_perm_id',
  ...
  const historyLatestActiveColumns = [
    'did',
    'schema_id',
    'type',
    'permission_id',
  ...
  const phSchemaEventHeightPermissionColumns = [
    'schema_id',
    'event_type',
    'height',
    'permission_id',
  ...
  const [
    hasLiveColumns,
    hasLiveCorporationQueryColumns,
    ...
    hasHistoryByCorporationColumns,
    hasHistoryByValidatorColumns,
    hasSessionHistoryRankingColumns,
    hasCsHistoryRankingColumns,
    hasTrHistoryRankingColumns,
    hasTdHistoryRankingColumns,
    hasHistoryCorporationSchemaColumns,
    hasCsTrArchivedModifiedColumns,
    hasCsModesColumns,
    hasCsParticipantsColumns,
    hasCsWeightColumns,
    hasCsIssuedColumns,
    hasCsVerifiedColumns,
    hasCsEcoSlashColumns,
    hasCsNetSlashColumns,
    hasTrCorporationArchivedModifiedColumns,
    hasTrParticipantsColumns,
    hasTrActiveSchemasColumns,
    hasTrWeightColumns,
    hasTrIssuedColumns,
    hasTrVerifiedColumns,
    hasTrEcoSlashColumns,
    hasTrNetSlashColumns,
    hasGlobalMetricsLookupColumns,
    hasPermSessionsModifiedColumns,
    hasPermSessionHistorySessionHeightModifiedColumns,
    hasTrHistoryCorporationHeightModifiedColumns,
    hasGfvHistoryTrHeightColumns,
    hasGfvHistoryTrVersionHeightColumns,
    hasGfdHistoryGfvTrHeightColumns,
    hasGfdHistoryTrHeightColumns,
    hasModuleParamsHistoryModuleHeightColumns,
    hasPermissionsCountryColumns,
    hasPhSchemaPermissionHeightColumns,
    hasCshTrSchemaHeightColumns,
    hasPhSchemaEventHeightPermissionColumns,
    hasStatsEntityGranularityIdTimeColumns,
    hasStatsGlobalGranularityTimeColumns,
  ] = await Promise.all([
  ...
  if (pg) {
    ...
    if (hasCsParticipantsColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${CS_LIVE_PARTICIPANTS_IDX} ON credential_schemas (participants, id DESC)`
      )
    if (hasCsWeightColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${CS_LIVE_WEIGHT_IDX} ON credential_schemas (weight, id DESC)`
      )
    if (hasCsIssuedColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${CS_LIVE_ISSUED_IDX} ON credential_schemas (issued, id DESC)`
      )
    if (hasCsVerifiedColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${CS_LIVE_VERIFIED_IDX} ON credential_schemas (verified, id DESC)`
      )
    if (hasCsEcoSlashColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${CS_LIVE_ECO_SLASH_EVENTS_IDX} ON credential_schemas (ecosystem_slash_events, id DESC)`
      )
    if (hasCsNetSlashColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${CS_LIVE_NET_SLASH_EVENTS_IDX} ON credential_schemas (network_slash_events, id DESC)`
      )
    ...
    if (hasTrParticipantsColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${TR_LIVE_PARTICIPANTS_IDX} ON trust_registry (participants, id DESC)`
      )
    if (hasTrActiveSchemasColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${TR_LIVE_ACTIVE_SCHEMAS_IDX} ON trust_registry (active_schemas, id DESC)`
      )
    if (hasTrWeightColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${TR_LIVE_WEIGHT_IDX} ON trust_registry (weight, id DESC)`
      )
    if (hasTrIssuedColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${TR_LIVE_ISSUED_IDX} ON trust_registry (issued, id DESC)`
      )
    if (hasTrVerifiedColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${TR_LIVE_VERIFIED_IDX} ON trust_registry (verified, id DESC)`
      )
    if (hasTrEcoSlashColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${TR_LIVE_ECO_SLASH_EVENTS_IDX} ON trust_registry (ecosystem_slash_events, id DESC)`
      )
    if (hasTrNetSlashColumns)
      await knex.raw(
        `CREATE INDEX CONCURRENTLY IF NOT EXISTS ${TR_LIVE_NET_SLASH_EVENTS_IDX} ON trust_registry (network_slash_events, id DESC)`
      )
