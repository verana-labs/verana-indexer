  ...
  if (!has) {
    ...
    try {
      await knex.schema.raw(
        `CREATE INDEX IF NOT EXISTS trust_deref_cache_expires_at_idx ON trust_deref_cache (expires_at)`
      )
    } catch {
      console.warn(
        'Failed to create index trust_deref_cache_expires_at_idx; this may impact performance of deref cache expiration.'
      )
    }
  } else {
    try {
...
export async function down(knex: Knex): Promise<void> {
  try {
    ...
    console.warn(
      'Failed to drop index trust_deref_cache_expires_at_idx; this may impact performance of deref cache expiration.'
    )
  }
