...
export default class PermissionSession extends BaseModel {
    ...
    id!: string; // UUID
    controller!: string; // account address
    agent_perm_id!: string;
    wallet_agent_perm_id!: string;
    authz!: AuthzEntry[]; // Array of authorization entries
    created!: string;
    modified!: string;
    ...
    static get jsonSchema() {
        return {
            type: 'object',
            required: ['id', 'controller', 'agent_perm_id', 'wallet_agent_perm_id', 'authz'],
            properties: {
                id: { type: 'string' },
    ...
    // Helper method to add authorization entry
    ...
    // Helper method to check if session has authorization for specific permissions
    hasAuthzFor(issuerPermId?: string, verifierPermId?: string): boolean {
        return this.authz.some(entry =>
