...
export default class MetricsApiService extends BaseService {
    ...
    if (!this.historyMetricColumnsAvailabilityPromise) {
      this.historyMetricColumnsAvailabilityPromise = (async () => {
        ...
        const checks = await Promise.all(
          requiredCsColumns.map((column) => knex.schema.hasColumn('credential_schema_history', column))
        )
  ...
  async getAll(ctx: Context) {
    ...
    try {
      ...
      const { csHasMetricColumns } = await this.getHistoryMetricColumnsAvailability().catch(() => ({
      ...
      const csLatest = IS_PG_CLIENT
        ? knex('credential_schema_history as csh')
            ...
            .select('csh.credential_schema_id', 'csh.archived', ...csMetricColumns)
      ...
      if (csHasMetricColumns) {
        metricAgg = await knex
          ...
          .select(
            ...
            knex.raw(
              'COALESCE(SUM(CAST(ecosystem_slashed_amount_repaid AS NUMERIC)), 0) as ecosystem_slashed_amount_repaid'
            ),
            ...
            knex.raw(
              'COALESCE(SUM(CAST(network_slashed_amount_repaid AS NUMERIC)), 0) as network_slashed_amount_repaid'
            )
          )
          .first()
      }
      ...
      if (IS_PG_CLIENT) {
        ...
        participantsAgg = await activeParticipantBaseQuery.clone().count('* as participants').first()
