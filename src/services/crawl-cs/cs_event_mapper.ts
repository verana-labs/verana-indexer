...
export function getDeterministicEvent(events: any[] | undefined) {
  if (!Array.isArray(events) || events.length === 0) return null

  const filtered = events.filter((event) => typeof event?.type === 'string' && CS_EVENT_TYPES.includes(event.type))
  ...
  const normalize = (event: any) =>
    JSON.stringify(Object.fromEntries((event.attributes ?? []).map((a: any) => [a.key, a.value])))
...
export function parseCredentialSchemaEvent(
  ...
  return {
    ...
    verifier_grantor_validation_validity_period: num(
      getAttr(attributeMap, 'verifier_grantor_validation_validity_period')
    ),
