...
function getValidityPeriod(
  ...
  schemaMessage: CredentialSchemaMessage | Record<string, unknown> | undefined
): number {
  const snakeCase = fieldName
  const camelCase = fieldName.replace(/_([a-z])/g, (_, letter) => letter.toUpperCase())

  const value = content?.[snakeCase] ?? content?.[camelCase] ?? schemaMessage?.[snakeCase] ?? schemaMessage?.[camelCase]
...
export default class ProcessCredentialSchemaService extends BullableService {
  ...
  async handleCredentialSchemas(ctx: Context<{ credentialSchemaMessages: CredentialSchemaMessage[] }>) {
    ...
    const processMessage = async (schemaMessageParams: CredentialSchemaMessage) => {
      const schemaMessage = enrichSchemaMessageWithEvent(schemaMessageParams, schemaMessageParams.txResponse)
      if (schemaMessage.type === VeranaCredentialSchemaMessageTypes.CreateCredentialSchema) {
    ...
    await this.processorBase.processInBatches(credentialSchemaMessages, processMessage, {
  ...
  private async createSchema(ctx: Context, schemaMessage: CredentialSchemaMessage) {
    try {
      ...
      const jsonSchemaForDb = typeof jsonSchema === 'string' ? jsonSchema : JSON.stringify(jsonSchema ?? {})
      ...
      const payload: Record<string, any> = {
        ...
        archived: null,
        is_active: true,
        issuer_grantor_validation_validity_period: getValidityPeriod(
          'issuer_grantor_validation_validity_period',
          content,
          schemaMessage
        ),
        verifier_grantor_validation_validity_period: getValidityPeriod(
          'verifier_grantor_validation_validity_period',
          content,
          schemaMessage
        ),
        issuer_validation_validity_period: getValidityPeriod(
          'issuer_validation_validity_period',
          content,
          schemaMessage
        ),
        verifier_validation_validity_period: getValidityPeriod(
          'verifier_validation_validity_period',
          content,
          schemaMessage
        ),
        holder_validation_validity_period: getValidityPeriod(
      ...
      const insertResult = await ctx.call(`${SERVICE.V1.CredentialSchemaDatabaseService.path}.upsert`, { payload })

      const generatedId =
        (insertResult as any)?.data?.result?.id ?? (insertResult as any)?.result?.id ?? (insertResult as any)?.id
  ...
  private async updateSchema(ctx: Context, schemaMessage: CredentialSchemaMessage) {
    try {
      const content = schemaMessage.content ?? {}
      const payload: Record<string, any> = {
  ...
  private async archiveSchema(ctx: Context, schemaMessage: CredentialSchemaMessage) {
    try {
      ...
      const archive = eventData ? eventData.archived != null : (schemaMessage.content as any)?.archive === true
      const modified = eventData?.modified ?? formatTimestamp(schemaMessage.timestamp) ?? new Date()
      const height = schemaMessage.height ?? 0

      await ctx.call(`${SERVICE.V1.CredentialSchemaDatabaseService.path}.archive`, {
        payload: {
          id: Number(id),
          archive,
