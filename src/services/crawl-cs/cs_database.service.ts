...
async function checkHistoryMetricColumnsExist(): Promise<boolean> {
  ...
  try {
    ...
    const checks = await Promise.all(
      requiredColumns.map((col) => knex.schema.hasColumn('credential_schema_history', col))
    )
...
export async function insertCredentialSchemaHistoryStatsRow(
  ...
  const historyRow = mapToHistoryRow(
    schemaRow,
    {
...
function normalizeArchivedValue(value: unknown): string | null {
  ...
  if (typeof value === 'string') {
    ...
    if (lowered === 'null' || lowered === 'undefined' || lowered === 'none' || lowered === 'false' || lowered === '0') {
      return null
    }
...
export default class CredentialSchemaDatabaseService extends BullableService {
      ...
      const result = await knex.transaction(async (trx) => {
        ...
        if (Number.isInteger(numericBlockchainId) && numericBlockchainId > 0) {
          ...
          if (existingSchemas.length > 0) {
            ...
            this.logger.info(
              `Found existing schema with blockchain_id=${blockchainSchemaId} (database_id=${existingSchema.id}). Updating instead of inserting.`
            )
          }
        }
        ...
        if (existingSchema) {
          ...
          const updatesForDb = await stripCsV4OptionalFields(trx, 'credential_schemas', updatesAligned)
          ...
          const insertForDb = await stripCsV4OptionalFields(trx, 'credential_schemas', insertAligned)
          ...
          const [inserted] = await trx('credential_schemas').insert(insertFiltered).returning('*')
        ...
        try {
          const { title: titleFromSchema, description: descriptionFromSchema } = extractTitleDescriptionFromJsonSchema(
            finalRecord?.json_schema
          )
          ...
          if (Object.keys(metaUpdates).length > 0) {
            const [updatedWithMeta] = await trx('credential_schemas')
        ...
        try {
          ...
          this.logger.warn(
            `Failed to calculate stats for CS ${finalRecord.id} at height ${blockHeight}: ${statsError?.message || String(statsError)}`
          )
        ...
        try {
          await trx('credential_schemas').where('id', finalRecord.id).update(getCSStatsUpdateObject(stats))
          await syncEcosystemStatsAndHistoryFromSchemaChange(trx, finalRecord.ecosystem_id, blockHeight)
        } catch (statsError: any) {
          this.logger.warn(
            ` Failed to update statistics for CS ${finalRecord.id}: ${statsError?.message || String(statsError)}`
          )
        }
  ...
  async update(ctx: Context<{ payload: any }>) {
    try {
      ...
      const updatesForDb = await stripCsV4OptionalFields(knex, 'credential_schemas', updatesAligned)
      ...
      let [updated] = await knex('credential_schemas').where({ id: existing.id }).update(updatesFiltered).returning('*')
      ...
      try {
        ...
        if (descriptionFromSchema !== null && descriptionFromSchema !== updated.description)
          metaUpdates.description = descriptionFromSchema
        if (Object.keys(metaUpdates).length > 0) {
      ...
      try {
        ...
        this.logger.warn(
          `Failed to calculate stats for CS ${existing.id} at height ${blockHeight}: ${statsError?.message || String(statsError)}`
        )
        stats = getDefaultCSStats()
      }
      ...
      try {
        await knex('credential_schemas').where('id', existing.id).update(getCSStatsUpdateObject(stats))
        await syncEcosystemStatsAndHistoryFromSchemaChange(knex, updated.ecosystem_id, blockHeight)
      } catch (statsError: any) {
        this.logger.warn(
          ` Failed to update statistics for CS ${existing.id}: ${statsError?.message || String(statsError)}`
        )
      }
  ...
  async archive(ctx: Context<{ payload: any }>) {
    try {
      ...
      const [updated] = await knex('credential_schemas').where({ id }).update(updates).returning('*')
      ...
      const hasHeightColumn = await checkHeightColumnExists()
      const _hasHistoryMetricColumns = await checkHistoryMetricColumnsExist()
      ...
      try {
        ...
        this.logger.warn(
          `Failed to calculate stats for CS ${id} at height ${blockHeight}: ${statsError?.message || String(statsError)}`
        )
        stats = getDefaultCSStats()
      }
      ...
      try {
        await knex('credential_schemas').where('id', id).update(getCSStatsUpdateObject(stats))
        await syncEcosystemStatsAndHistoryFromSchemaChange(knex, updated.ecosystem_id, blockHeight)
      } catch (statsError: any) {
  ...
  async syncFromLedger(ctx: Context<{ ledgerResponse: { schema?: Record<string, unknown> }; blockHeight: number }>) {
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        issuer_grantor_validation_validity_period: Number(
          schema.issuer_grantor_validation_validity_period ?? schema.issuerGrantorValidationValidityPeriod ?? 0
        ),
        verifier_grantor_validation_validity_period: Number(
          schema.verifier_grantor_validation_validity_period ?? schema.verifierGrantorValidationValidityPeriod ?? 0
        ),
        issuer_validation_validity_period: Number(
          schema.issuer_validation_validity_period ?? schema.issuerValidationValidityPeriod ?? 0
        ),
        verifier_validation_validity_period: Number(
          schema.verifier_validation_validity_period ?? schema.verifierValidationValidityPeriod ?? 0
        ),
        holder_validation_validity_period: Number(
          schema.holder_validation_validity_period ?? schema.holderValidationValidityPeriod ?? 0
        ),
        issuer_onboarding_mode: String(
          schema.issuer_onboarding_mode ?? schema.issuerOnboardingMode ?? 'MODE_UNSPECIFIED'
        ),
        verifier_onboarding_mode: String(
          schema.verifier_onboarding_mode ?? schema.verifierOnboardingMode ?? 'MODE_UNSPECIFIED'
        ),
        holder_onboarding_mode:
      ...
      const payloadForDb = await stripCsV4OptionalFields(knex, 'credential_schemas', payloadAligned)
      const updates: Record<string, unknown> = { ...payloadForDb }
      delete (updates as Record<string, unknown>).id
      if (updates.json_schema != null) {
        ;(updates as Record<string, unknown>).json_schema = overrideSchemaIdInString(String(updates.json_schema), id)
      }
      ...
      if (existing) {
        ...
        if (Object.keys(changedUpdates).length === 0) {
          ...
          const [updated] = await knex('credential_schemas').where({ id }).update(changedUpdates).returning('*')
          if (!updated) {
        ...
        ;(insertPayload as Record<string, unknown>).json_schema =
          (insertPayload as Record<string, unknown>).json_schema ?? '{}'
        await ensureDepositDefaultIfColumnExists(
          knex,
          'credential_schemas',
        ...
        const [inserted] = await knex('credential_schemas').insert(insertPayload).returning('*')
        if (!inserted) {
      ...
      try {
        ...
        this.logger.warn(
          `Failed to persist title/description for CS ${finalRecord.id} in syncFromLedger: ${err?.message || err}`
        )
      }
      const hasHeightColumn = await checkHeightColumnExists()
      ...
      if (!existing || Object.keys(historyChangesForUpdate).length > 0) {
        ...
        try {
          ...
          this.logger.warn(
            `Failed to calculate stats for CS ${id} at height ${blockHeightNum} in syncFromLedger: ${statsError?.message || String(statsError)}`
          )
          stats = {
            participants: 0,
            participants_ecosystem: 0,
        ...
        try {
          ...
          this.logger.warn(
            `Failed to update stats for CS ${id} in syncFromLedger: ${statsUpdateError?.message || String(statsUpdateError)}`
          )
        }
      }
    ...
    try {
      ...
      if (typeof blockHeight === 'number') {
        ...
        if (participantAccount) {
          ...
          if (hasHeightColumn) {
            ...
            subquery = knex('credential_schema_history')
              .select('credential_schema_id')
              .select(knex.raw(`ROW_NUMBER() OVER (PARTITION BY credential_schema_id ORDER BY created_at DESC) as rn`))
              .as('ranked')
          }

          const latestHistory = await knex.from(subquery).select('credential_schema_id').where('rn', 1)
        ...
        // biome-ignore lint/suspicious/noImplicitAnyLet: type is inferred from the .map() assignments below
        ...
        if (typeof blockHeight === 'number') {
          if (hasHistoryMetricColumns) {
            ...
            const statsRows = await knex('credential_schema_history as csh')
              ...
              .whereIn(
                'csh.credential_schema_id',
                filteredItems.map((i) => Number(i.id))
              )
        ...
        filteredWithStats = applyHalfOpenRangeToRows(
          ...
          (s) => toFiniteNumber(s.ecosystem_slash_events)
        )
        filteredWithStats = applyHalfOpenRangeToRows(
          filteredWithStats,
          minNetworkSlashEvents,
          maxNetworkSlashEvents,
          (s) => toFiniteNumber(s.network_slash_events)
        )
      ...
      const schemasWithStats = items.map((item) => {
        ...
        return {
          ...
          participants_ecosystem:
            typeof (item as any).participants_ecosystem === 'number'
              ? (item as any).participants_ecosystem
              : Number((item as any).participants_ecosystem || 0),
          participants_issuer_grantor:
            typeof (item as any).participants_issuer_grantor === 'number'
              ? (item as any).participants_issuer_grantor
              : Number((item as any).participants_issuer_grantor || 0),
          participants_issuer:
            typeof (item as any).participants_issuer === 'number'
              ? (item as any).participants_issuer
              : Number((item as any).participants_issuer || 0),
          participants_verifier_grantor:
            typeof (item as any).participants_verifier_grantor === 'number'
              ? (item as any).participants_verifier_grantor
              : Number((item as any).participants_verifier_grantor || 0),
          participants_verifier:
            typeof (item as any).participants_verifier === 'number'
              ? (item as any).participants_verifier
              : Number((item as any).participants_verifier || 0),
          participants_holder:
          ...
          ecosystem_slash_events:
            typeof item.ecosystem_slash_events === 'number'
              ? item.ecosystem_slash_events
              : Number(item.ecosystem_slash_events || 0),
          ecosystem_slashed_amount:
            typeof item.ecosystem_slashed_amount === 'number'
              ? item.ecosystem_slashed_amount
              : Number(item.ecosystem_slashed_amount || 0),
          ecosystem_slashed_amount_repaid:
            typeof item.ecosystem_slashed_amount_repaid === 'number'
              ? item.ecosystem_slashed_amount_repaid
              : Number(item.ecosystem_slashed_amount_repaid || 0),
          network_slash_events:
            typeof item.network_slash_events === 'number'
              ? item.network_slash_events
              : Number(item.network_slash_events || 0),
          network_slashed_amount:
            typeof item.network_slashed_amount === 'number'
              ? item.network_slashed_amount
              : Number(item.network_slashed_amount || 0),
          network_slashed_amount_repaid:
            typeof item.network_slashed_amount_repaid === 'number'
              ? item.network_slashed_amount_repaid
              : Number(item.network_slashed_amount_repaid || 0),
        }
      })
  ...
  private async getCredentialSchemaIdsForParticipant(account: string): Promise<number[]> {
    ...
    const schemaIdsFromController =
      controllerEcosystemIds.length === 0
        ? []
        : (await knex('credential_schemas').whereIn('ecosystem_id', controllerEcosystemIds).select('id')).map(
            (r: { id: number }) => r.id
          )
  ...
  private async getCredentialSchemaIdsForParticipantAtHeight(account: string, blockHeight: number): Promise<number[]> {
    ...
    if (controllerEcosystemIds.length > 0) {
      const cshRanked = knex('credential_schema_history')
        ...
        .where('height', '<=', blockHeight)
        .as('ranked')
      const latestCsh = await knex
        .from(cshRanked)
        .where('rn', 1)
        .whereIn('ecosystem_id', controllerEcosystemIds)
        .select('credential_schema_id')
