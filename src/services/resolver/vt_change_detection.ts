...
function mergeParticipations(cur: VtRawChange['participations'], cats: ParticipationsCats): ParticipationsCats {
...
export async function listVtChangeHeights(fromBlock: number, toBlock: number, limit: number): Promise<number[]> {
  ...
  const rows = (await knex.from(union.as('t')).distinct('height').orderBy('height', 'asc').limit(limit)) as Array<{
...
export async function buildVtChangesForBlock(blockHeight: number): Promise<VtRawChange[]> {
  ...
  const validatorIds = [
    ...new Set(
      participantRows.map((r) => Number(r.validator_participant_id)).filter((id) => Number.isInteger(id) && id > 0)
    ),
  ]
  ...
  if (trustRows.length > 0) {
    ...
    if (trustDids.length > 0) {
      const corps = (await knex('corporation').select('did', 'id').whereIn('did', trustDids)) as Array<{
        did: string
        id: number
      }>
