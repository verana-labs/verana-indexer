...
export function parseIndexedDidsLimit(raw: unknown): { ok: true; value: number } | { ok: false; error: string } {
...
export function decodeOffsetCursor(raw: unknown): { ok: true; value: number } | { ok: false; error: string } {
...
function universeSubqueries(atBlock: number) {
  const base = (table: string) => knex(table).distinct('did').whereNotNull('did').andWhere('height', '<=', atBlock)
  return [base('corporation_history'), base('ecosystem_history'), base('participant_history'), base('trust_results')]
}
...
function corporationSubqueries(atBlock: number, corporationId: number) {
  ...
  return [scoped('corporation_history'), scoped('ecosystem_history'), scoped('participant_history'), validatorBranch]
}
...
export async function listIndexedDidsPage(opts: {
  ...
  const subqueries =
    corporationId === null ? universeSubqueries(atBlock) : corporationSubqueries(atBlock, corporationId)
