...
export async function buildCorporation(
  did: string,
  atHeight?: number
...
const IS_PG = String(
  (knex as { client?: { config?: { client?: string } } }).client?.config?.client || ""
).includes("pg");
...
async function fetchVpCredentials(endpoint: string): Promise<Array<Record<string, unknown>>> {
  ...
  try {
    ...
    const vc = (vp?.verifiableCredential ?? (vp as { verifiableCredentials?: unknown })?.verifiableCredentials) as unknown;
...
async function resolveParticipantId(did: string, credentialSchemaId: number): Promise<number> {
  ...
  const row = (await knex("participants")
    .where({ did, schema_id: credentialSchemaId })
    .select("id")
  ...
  for (const svc of didDocumentServices(resolveResult)) {
    ...
    for (const cred of credentials) {
      ...
      vtcCredentials.push({
        ...
        issuerParticipantId: issuerDid
          ? await resolveIssuerParticipantId(issuerDid, schema.credentialSchemaId)
          : 0,
      });
    }
