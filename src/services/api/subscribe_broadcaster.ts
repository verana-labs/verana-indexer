...
import { buildBlockEnvelope, type ControlMessage, parseControlMessage } from './subscribe_protocol'
...
export class SubscribeBroadcaster extends BaseSubscribeServer<ControlMessage, ClientState> {
  ...
  broadcastBlockEnvelope(args: { block: number; blockTime: string; events: IndexerEventRecord[] }): void {
    ...
    this.clients.forEach((state, ws) => {
      ...
      const envelope = buildBlockEnvelope(args.block, args.blockTime, args.events, state.dids, state.corporationId)
      if (this.sendJson(ws, envelope as unknown as Record<string, unknown>)) sent++
    })

    if (sent > 0) {
      this.logger.info(`[SubscribeBroadcaster] Broadcasted block ${args.block} to ${sent} subscriber(s)`)
    }
  }
}
