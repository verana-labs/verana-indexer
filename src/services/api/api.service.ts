...
async function fetchBlockCheckpoint() {
  return knex('block_checkpoint').select('height', 'updated_at').where('job_name', BLOCK_CHECKPOINT_JOB).first()
}
...
function getHeaderValue(req: IncomingMessage): string | null {
  ...
  const headerKey = Object.keys(req.headers).find((key) => key.toLowerCase() === normalizedHeader)
  if (headerKey) {
...
async function parseAtBlockHeight(ctx: Context<any, any>, req: IncomingMessage, required: boolean = false) {
  ctx.meta = ctx.meta || {}
  const headerValue = getHeaderValue(req)

  if (!headerValue) {
    if (required) {
      throw new Errors.MoleculerError('Missing At-Block-Height header', 400, 'AT_BLOCK_HEIGHT_REQUIRED')
    }
    return
  }
  ...
  try {
    ...
    if (checkpoint) {
      ...
      if (checkpoint.updated_at) {
        const updatedAt =
          checkpoint.updated_at instanceof Date ? checkpoint.updated_at : new Date(checkpoint.updated_at)
...
function createOnBeforeCall(required: boolean = true) {
  return async (ctx: Context<any, any>, _route: Route, req: IncomingMessage) => {
    try {
      ...
      const error =
        err instanceof Errors.MoleculerError
          ? err
          : new Errors.MoleculerError(err?.message || 'Internal error', err?.code || 500, err?.type || 'INTERNAL_ERROR')
      throw error
    }
  }
}
...
function createOnError() {
  return async (req: IncomingMessage, res: ServerResponse, err: any) => {
    try {
...
function createOnAfterCall() {
  return async (_ctx: Context<any, any>, _route: Route, req: IncomingMessage, res: ServerResponse, data: any) => {
...
function createRoute(path: string, aliases: Record<string, string>, requireBlockHeight: boolean = false) {
  return {
    path,
    aliases,
