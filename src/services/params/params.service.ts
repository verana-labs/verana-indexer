...
import { BULL_JOB_NAME, getHttpBatchClient, ModulesParamsNamesTypes, SERVICE } from '../../common'
...
export default class GenesisParamsService extends BullableService {
  ...
  public async handleUpdateParams(
    ctx: Context<{
      message: any
      height: number
      txHash?: string
  ...
  private async fetchGenesisFromNetwork(): Promise<void> {
    try {
      ...
      try {
        const genesisResponse = await httpBatchClient.execute(createJsonRpcRequest('genesis'))

        fs.writeFileSync(this.genesisPath, JSON.stringify(genesisResponse.result.genesis, null, 2), 'utf-8')
        ...
        if (chunks.length > 0) {
          ...
          try {
            ...
            fs.writeFileSync(this.genesisPath, JSON.stringify(parsedGenesis, null, 2), 'utf-8')
  ...
  public async resetGenesisCheckpoints(): Promise<void> {
    try {
      ...
      if (affected > 0) {
        this.logger.info(`🔁 Reset ${affected} block_checkpoint rows (height=0) for genesis-related jobs`)
      } else {
  ...
  private async processUpdateParams(message: any, height: number, txHash?: string): Promise<any> {
    ...
    if (type === VeranaCredentialSchemaMessageTypes.UpdateParams) {
      ...
      module = ModulesParamsNamesTypes.GF
    } else {
  ...
  private async updateModuleParams(
    ...
    try {
      ...
      await recordModuleParamsHistorySafe(trx, module, params, 'UPDATE_PARAMS', height, previousParams)
