...
export default class StatsCalculationService extends BullableService {
  ...
  private async calculateGlobalStats(granularity: Granularity, timestamp: Date): Promise<void> {
    try {
      ...
      const existing = await Stats.query()
        ...
        .first()
      this.logger.info(
        `GLOBAL [${granularity}] Existing stats check result: ${existing ? `Found existing entry (id: ${existing.id})` : 'No existing entry'}`
      )

      this.logger.info(`GLOBAL [${granularity}] Querying participants table...`)
      const participantCount = await knex('participants').where('created', '<=', timestamp).count('* as count').first()
      ...
      if (existing) {
        ...
        if (!this.hasAnyDelta(stats)) {
          ...
          const updateResult = await Stats.query().findById(existing.id).patch(stats)
          this.logger.info(`GLOBAL [${granularity}] Update query executed. Result:`, updateResult)
          this.logger.info(
            `GLOBAL [${granularity}]  Successfully updated stats for ${granularity} at ${timestamp.toISOString()}`
          )
        }
      } else {
        ...
        if (!hasAnyDelta) {
          ...
          try {
            ...
            this.logger.info(
              `GLOBAL [${granularity}]  Successfully created stats for ${granularity} at ${timestamp.toISOString()} (id: ${inserted.id})`
            )
            ...
            if (isUniqueViolation) {
              this.logger.warn(
                `GLOBAL [${granularity}]  Duplicate key detected (race condition), fetching existing record and updating...`
              )
              ...
              if (existingRecord) {
                if (!this.hasAnyDelta(stats)) {
                  ...
                  await Stats.query().findById(existingRecord.id).patch(stats)
                  this.logger.info(
                    `GLOBAL [${granularity}]  Updated existing stats record (id: ${existingRecord.id}) after duplicate key detection`
                  )
                }
              } else {
                this.logger.warn(
                  `GLOBAL [${granularity}]  Duplicate key detected but record not found (likely race condition - non-critical)`
                )
              }
            } else {
      ...
      for (const ec of ecosystems) {
        try {
          ...
          if (existing) {
            ...
            if (!this.hasAnyDelta(stats)) {
              this.logger.info(
                `[ECOSYSTEM][${granularity}] SKIPPING UPDATE for EC ${ec.id} - all delta fields are zero`
              )
            } else if (this.isStatsEqual(existing, stats)) {
              this.logger.info(`[ECOSYSTEM][${granularity}] SKIPPING UPDATE for EC ${ec.id} - no changes`)
            } else {
              await Stats.query().findById(existing.id).patch(stats)
            ...
            if (!this.hasAnyDelta(stats)) {
              this.logger.info(
                `[ECOSYSTEM][${granularity}] SKIPPING INSERT for EC ${ec.id} - all delta fields are zero`
              )
              continue
            }
            try {
              ...
              if (isUniqueViolation) {
                this.logger.warn(
                  `[ECOSYSTEM][${granularity}]  Duplicate key detected for EC ${ec.id} (race condition), fetching existing record and updating...`
                )
                const existingRecord = await Stats.query()
                  .where('granularity', granularity)
                  .where('timestamp', timestamp.toISOString())
                ...
                if (existingRecord) {
                  if (!this.hasAnyDelta(stats)) {
                    ...
                    await Stats.query().findById(existingRecord.id).patch(stats)
                    this.logger.info(
                      `[ECOSYSTEM][${granularity}]  Updated existing stats for EC ${ec.id} (id: ${existingRecord.id}) after duplicate key detection`
                    )
                  }
                } else {
                  ...
                  this.logger.debug(
                    `[ECOSYSTEM][${granularity}]  Duplicate key detected for EC ${ec.id} but record not found (likely race condition - non-critical)`
                  )
                }
              } else {
    ...
    try {
      ...
      for (const schema of schemas) {
        try {
          ...
          if (existing) {
            this.logger.info(
              `CREDENTIAL_SCHEMA[${granularity}] Updating existing entry for schema ${schema.id} (id: ${existing.id})`
            )
            if (!this.hasAnyDelta(stats)) {
              this.logger.info(
                `CREDENTIAL_SCHEMA[${granularity}] SKIPPING UPDATE for schema ${schema.id} - all delta fields are zero`
              )
            } else if (this.isStatsEqual(existing, stats)) {
            ...
            try {
              ...
              created++
              this.logger.info(
                `CREDENTIAL_SCHEMA[${granularity}]  Created stats for schema ${schema.id} (id: ${inserted.id})`
              )
            } catch (insertError: any) {
              ...
              if (isUniqueViolation) {
                this.logger.warn(
                  `CREDENTIAL_SCHEMA[${granularity}]  Duplicate key detected for schema ${schema.id} (race condition), fetching existing record and updating...`
                )
                const existingRecord = await Stats.query()
                  .where('granularity', granularity)
                  .where('timestamp', timestamp.toISOString())
                ...
                if (existingRecord) {
                  if (!this.hasAnyDelta(stats)) {
                    ...
                    this.logger.info(
                      `CREDENTIAL_SCHEMA[${granularity}]  SKIPPING UPDATE for schema ${schema.id} - no changes`
                    )
                  } else {
                    await Stats.query().findById(existingRecord.id).patch(stats)
                    updated++
                    this.logger.info(
                      `CREDENTIAL_SCHEMA[${granularity}]  Updated existing stats for schema ${schema.id} (id: ${existingRecord.id}) after duplicate key detection`
                    )
                  }
                } else {
                  // Race condition: Another process inserted and possibly deleted the record, or there's a timing issue
                  // This is not critical - the duplicate key violation prevented the duplicate insert, which is the desired behavior
                  this.logger.debug(
                    `CREDENTIAL_SCHEMA[${granularity}]  Duplicate key detected for schema ${schema.id} but record not found (likely race condition - non-critical)`
                  )
                }
              } else {
          ...
          this.logger.error(
            `CREDENTIAL_SCHEMA Error processing schema ${schema.id}:`,
            error?.message || error,
            error?.stack
          )
        }
      ...
      this.logger.error(
        `CREDENTIAL_SCHEMA Error in calculateCredentialSchemaStats:`,
        error?.message || error,
        error?.stack
      )
      throw error
    ...
    try {
      ...
      for (const participant of participants) {
        try {
          ...
          if (existing) {
            this.logger.info(
              `PARTICIPANT [${granularity}] Updating existing entry for participant ${participant.id} (id: ${existing.id})`
            )
            if (!this.hasAnyDelta(stats)) {
              this.logger.info(
                `PARTICIPANT [${granularity}] SKIPPING UPDATE for participant ${participant.id} - all delta fields are zero`
              )
            } else if (this.isStatsEqual(existing, stats)) {
              this.logger.info(
                `PARTICIPANT [${granularity}] SKIPPING UPDATE for participant ${participant.id} - no changes`
              )
            } else {
              await Stats.query().findById(existing.id).patch(stats)
              updated++
            ...
            if (!this.hasAnyDelta(stats)) {
              skipped++
              this.logger.debug(
                `PARTICIPANT SKIPPING INSERT for participant ${participant.id} - all delta fields are zero`
              )
              continue
            }
            try {
              ...
              created++
              this.logger.info(
                `PARTICIPANT [${granularity}]  Created stats for participant ${participant.id} (id: ${inserted.id})`
              )
            } catch (insertError: any) {
              const isUniqueViolation =
                insertError?.nativeError?.code === '23505' ||
              ...
              if (isUniqueViolation) {
                this.logger.warn(
                  `PARTICIPANT [${granularity}]  Duplicate key detected for participant ${participant.id} (race condition), fetching existing record and updating...`
                )
                const existingRecord = await Stats.query()
                  .where('granularity', granularity)
                  .where('timestamp', timestamp.toISOString())
                ...
                if (existingRecord) {
                  if (!this.hasAnyDelta(stats)) {
                    ...
                    updated++
                    this.logger.info(
                      `PARTICIPANT [${granularity}]  Updated existing stats for participant ${participant.id} (id: ${existingRecord.id}) after duplicate key detection`
                    )
                  }
                } else {
                  // Race condition: Another process inserted and possibly deleted the record, or there's a timing issue
                  // This is not critical - the duplicate key violation prevented the duplicate insert, which is the desired behavior
                  this.logger.debug(
                    `PARTICIPANT [${granularity}]  Duplicate key detected for participant ${participant.id} but record not found (likely race condition - non-critical)`
                  )
                }
              } else {
          ...
          this.logger.error(
            `PARTICIPANT Error processing participant ${participant.id}:`,
            error?.message || error,
            error?.stack
          )
        }
  ...
  private async computeGlobalStats(timestamp: Date): Promise<any> {
    ...
    const delta = {
      ...
      ecosystem_slashed_amount:
        cumulative.ecosystem_slashed_amount - BigInt(prevStats?.cumulative_ecosystem_slashed_amount || '0'),
      ecosystem_slashed_amount_repaid:
        cumulative.ecosystem_slashed_amount_repaid -
        BigInt(prevStats?.cumulative_ecosystem_slashed_amount_repaid || '0'),
      network_slash_events: cumulative.network_slash_events - (prevStats?.cumulative_network_slash_events || 0),
      network_slashed_amount:
        cumulative.network_slashed_amount - BigInt(prevStats?.cumulative_network_slashed_amount || '0'),
      network_slashed_amount_repaid:
        cumulative.network_slashed_amount_repaid - BigInt(prevStats?.cumulative_network_slashed_amount_repaid || '0'),
    }
  ...
  private async processStatsFromCheckpoint(): Promise<void> {
    ...
    try {
      ...
      if (!handleTxCheckpoint) {
        this.logger.debug(
          'CHECKPOINT HANDLE_TRANSACTION checkpoint not found, waiting for transactions to be processed...'
        )
        return
      }
      ...
      const statsCheckpoint = await BlockCheckpoint.query().where('job_name', BULL_JOB_NAME.CALCULATE_STATS).first()
      ...
      if (statsHeight >= handleTxHeight) {
        this.logger.debug(
          `CHECKPOINT Stats already up to date (${statsHeight} >= ${handleTxHeight}), no new transactions to process`
        )
        return
      }

      const block = await Block.query().select('height', 'time').where('height', handleTxHeight).first()
      ...
      this.logger.debug(
        `CHECKPOINT Processing stats for HANDLE_TRANSACTION checkpoint height: ${handleTxHeight}, block timestamp: ${blockTimestamp.toISOString()}`
      )
      ...
      try {
        ...
        if (statsCheckpoint) {
          ...
          await BlockCheckpoint.query().insert(statsCheckpoint).onConflict('job_name').merge().returning('id')
        } else {
        ...
        this.logger.debug(
          `CHECKPOINT Updated stats checkpoint to height ${handleTxHeight} (synced with HANDLE_TRANSACTION)`
        )
      } catch (calcError: any) {
