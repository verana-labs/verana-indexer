...
import BullableService, { DEFAULT_PREFIX, QueueHandler } from '../../base/bullable.service'
import {
  BULL_JOB_NAME,
  Config,
  getHttpBatchClient,
...
export default class CrawlBlockService extends BullableService {
  ...
  private async jobHandler(_payload: unknown): Promise<void> {
    try {
      ...
      await handleErrorGracefully(error, SERVICE.V1.CrawlBlock.key, 'Unexpected error in job handler')
    }
  }
  ...
  private async initEnv() {
    try {
      ...
      if (wasStopped) {
        ...
        this.logger.warn(
          `LCD client initialization failed (non-critical): ${error?.message || error}. Service will continue and retry later.`
        )
        this._lcdClient = null as any
        return
      }
    ...
    try {
      ...
      if (
        ...
        this.logger.warn(
          `Failed to get node info due to network/timeout error (${err.code || 'timeout'}): ${err.message}. Continuing without SDK version update.`
        )
      } else {
    ...
    if (this._isFreshStart) {
      this.logger.info(
        `Fresh start detected: blocks=${startMode.totalBlocks}, checkpoint=${startMode.currentBlock}, cacheCleared=${startMode.cacheCleared || false}`
      )
    } else {
  ...
  async handleJobCrawlBlock() {
    ...
    try {
      ...
      try {
        await runWithCrawlLock(async () => {
          ...
          const isWebSocketActive =
            config.crawlBlock.enableWebSocketSubscription && this._websocketConnected && isCaughtUp
          ...
          if (
            ...
            if (latestHeight === 0) {
              ...
              this.logger.debug(
                `No new blocks (latest: ${latestHeight}, current: ${this._currentBlock}), skipping fetch`
              )
          ...
          const latestBlockNetwork = parseInt(responseGetLatestBlock.block.header.height.toString(), 10)
          ...
          if (
            ...
            if (Date.now() - this._lastHealthCheckTime > healthCheckInterval) {
              ...
              if (this._isFreshStart) {
                ...
                crawlDelay = getOptimalDelay(applySpeedToDelay(baseFreshDelay, false), this._lastHealthCheck, true)
                this.logger.info(
                  `🏥 Health: ${this._lastHealthCheck.overall} | DB: ${this._lastHealthCheck.database.connectionUsagePercent?.toFixed(1)}% | Memory: ${this._lastHealthCheck.server.memoryUsagePercent?.toFixed(1)}% | Using ${blocksPerCall} blocks/call, ${crawlDelay}ms delay`
                )
              } else {
                ...
                crawlDelay = getOptimalDelay(applySpeedToDelay(baseReindexDelay, true), this._lastHealthCheck, false)
          ...
          for (let i = startBlock; i <= endBlock; i += 1) {
            try {
              ...
              if (blockReq && blockResultsReq) {
                const blockRequest = blockReq
                const blockResultsRequest = blockResultsReq
                blockQueries.push(
                  this.executeRpcWithRetry(() => this._httpBatchClient.execute(blockRequest), `block-${heightStr}`),
                  this.executeRpcWithRetry(
                    () => this._httpBatchClient.execute(blockResultsRequest),
                    `block_results-${heightStr}`
                  )
                )
              }
            } catch (err) {
        ...
        if (e?.name === 'CrawlSkipError') {
          this.logger.warn(
            `[CRAWL_BLOCK] Skipping cycle: ${e?.message || 'crawler temporarily paused (DB pool or memory pressure).'}`
          )
          return
        }
        throw e
      ...
      if (isNetworkError) {
        this.logger.warn(
          `Network/timeout error in block crawling (${err.code || 'timeout'}): ${errorMessage}. Skipping this cycle, will retry on next polling interval`
        )
      } else {
        await handleErrorGracefully(error, SERVICE.V1.CrawlBlock.key, 'Error in block crawling')
      }
    } finally {
  ...
  private async getLatestBlockHeight(): Promise<number> {
    try {
      ...
      return parseInt(
        responseGetLatestBlock?.block?.header?.height ? responseGetLatestBlock.block.header.height.toString() : '0',
        10
      )
    } catch (error: unknown) {
      const err = error as NodeJS.ErrnoException
  ...
  private async executeRpcWithRetry<T>(rpcCall: () => Promise<T>, operationName: string): Promise<T> {
        ...
        try {
          ...
          if (insertError?.code === '23505' || insertError?.constraint === 'block_checkpoint_job_name_unique') {
            blockHeightCrawled = await BlockCheckpoint.query()
              ...
              .timeout(getDbQueryTimeoutMs())
            this.logger.info(
              `Checkpoint already exists (created by another service) at height ${blockHeightCrawled?.height || 0}`
            )
          } else {
            throw insertError
          }
      ...
      if (existPartition.rows.length === 0) {
        ...
        try {
          await knex.transaction(async (trx) => {
            await knex
              .raw(`CREATE TABLE ${initialPartitionName} (LIKE block INCLUDING ALL EXCLUDING CONSTRAINTS)`)
              .transacting(trx)
            await knex
              .raw(`ALTER TABLE block ATTACH PARTITION ${initialPartitionName} FOR VALUES FROM (0) TO (${step})`)
              .transacting(trx)
          })
  ...
  private async ensurePartitionsExist(listBlockModel: any[]): Promise<void> {
    ...
    const heights = listBlockModel
      .map((block: any) => parseInt(String(block.height || 0), 10))
      .filter((h: number) => h > 0)
    ...
    for (const partitionInfo of partitionsToCheck) {
      try {
        ...
        if (existPartition.rows.length === 0) {
          this.logger.info(
            `Creating missing partition: ${partitionInfo.partitionName} for heights ${partitionInfo.fromHeight}-${partitionInfo.toHeight}`
          )

          await knex.transaction(async (trx) => {
            await knex
              .raw(`CREATE TABLE ${partitionInfo.partitionName} (LIKE block INCLUDING ALL EXCLUDING CONSTRAINTS)`)
              .transacting(trx)
            await knex
              .raw(
  ...
  private async adjustPollingInterval(latestBlockNetwork: number): Promise<void> {
    ...
    if (targetInterval !== this._currentInterval) {
      ...
      try {
        ...
        await handleErrorGracefully(error, SERVICE.V1.CrawlBlock.key, 'Error updating job interval')
      } finally {
      ...
      listBlock.forEach((block) => {
        ...
        if (!mapExistedBlock.get(height)) {
          ...
          listBlockModel.push({
            ...
            signatures: block?.block?.last_commit?.signatures.map((signature: CommitSigSDKType) => ({
  ...
  private async ensureCrawlBlockJob(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const existingJobNoPrefix = repeatableJobsNoPrefix.find(
          (job: { name?: string }) => job.name === BULL_JOB_NAME.CRAWL_BLOCK
        )

        const repeatableJobsWithPrefix = await jobQueueWithPrefix.getRepeatableJobs()
        const existingJobWithPrefix = repeatableJobsWithPrefix.find(
          (job: { name?: string }) => job.name === BULL_JOB_NAME.CRAWL_BLOCK
        )

        if (existingJobNoPrefix || existingJobWithPrefix) {
          this.logger.info(
            `Crawl block job already exists (no prefix: ${!!existingJobNoPrefix}, with prefix: ${!!existingJobWithPrefix}), skipping creation`
          )
        ...
        try {
          ...
          const verifiedNoPrefix = verifyJobsNoPrefix.find(
            (job: { name?: string; id?: string; key?: string }) =>
              job.name === BULL_JOB_NAME.CRAWL_BLOCK ||
              job.id?.includes(BULL_JOB_NAME.CRAWL_BLOCK) ||
              job.key?.includes(BULL_JOB_NAME.CRAWL_BLOCK)
          )
          const verifiedWithPrefix = verifyJobsWithPrefix.find(
            (job: { name?: string; id?: string; key?: string }) =>
              job.name === BULL_JOB_NAME.CRAWL_BLOCK ||
              job.id?.includes(BULL_JOB_NAME.CRAWL_BLOCK) ||
              job.key?.includes(BULL_JOB_NAME.CRAWL_BLOCK)
          ...
          if (verifiedNoPrefix || verifiedWithPrefix) {
            ...
            this.logger.info(
              `Job creation initiated. Worker is registered and will process jobs. Repeatable job may take a moment to appear in Redis.`
            )
          }
        } catch (verifyError) {
          this.logger.info(
            `Could not verify job creation (non-critical): ${verifyError}. Worker is registered and will process jobs.`
          )
        }
      } finally {
        await redisClient.quit()
  ...
  public async _start() {
    ...
    this._isFreshStart = startMode.isFreshStart
    this.logger.info(
      `Start mode detection: totalBlocks=${startMode.totalBlocks}, currentBlock=${startMode.currentBlock}, isFreshStart=${this._isFreshStart}`
    )
  ...
  private getWebSocketUrl(): string {
    ...
    if (!rpcUrl) {
      ...
      handleErrorGracefully(error, SERVICE.V1.CrawlBlock.key, 'WebSocket URL configuration error').catch(() => {})
      throw error
    }
    ...
    try {
      ...
      this._websocket.on('message', async (data: WebSocket.Data) => {
        try {
          ...
          if (this._processingLock) {
            this.logger.debug(
              `WebSocket: Block ${blockHeight} detected but processing already in progress, will be handled by current cycle`
            )
            return
          }
      ...
      await handleErrorGracefully(error, SERVICE.V1.CrawlBlock.key, 'Error starting WebSocket subscription')
      this._websocketConnected = false
    }
  }
