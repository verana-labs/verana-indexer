...
import { extractStartParticipantOpNewParticipantId } from '../../modules/pp-height-sync/pp_height_sync_helpers'
...
export default class ParticipantProcessorService extends BullableService {
  ...
  async handleParticipantMessages(
    ...
    for (let i = 0; i < sortedMessages.length; i++) {
      ...
      try {
        this.logger.info(
          ` Processing Participant message ${i + 1}/${totalMessages}: type=${msg.type}, height=${msg.height}`
        )
        ...
        if (
          ...
          const newId = extractStartParticipantOpNewParticipantId(msg as ParticipantMessagePayload)
          if (newId != null) {
            ;(payload as any).id = newId
          }
        }
        if (msg.type === VeranaParticipantMessageTypes.StartParticipantOP && (payload as any)?.id == null) {
          const opNewId = extractStartParticipantOpNewParticipantId(msg as ParticipantMessagePayload)
          if (opNewId != null) {
        ...
        switch (msg.type) {
          ...
          case VeranaParticipantMessageTypes.StartParticipantOP:
            ...
            break
          case VeranaParticipantMessageTypes.SetParticipantOPToValidated:
            result = await this.broker.call('participantIngest.handleMsgSetParticipantOPToValidated', { data: payload })
          ...
          case VeranaParticipantMessageTypes.RenewParticipantOP:
            ...
            break
          case VeranaParticipantMessageTypes.CancelParticipantOPLastRequest:
            result = await this.broker.call('participantIngest.handleMsgCancelParticipantOPLastRequest', {
              data: payload,
            })
            break
          case VeranaParticipantMessageTypes.CreateOrUpdateParticipantSession:
            result = await this.broker.call('participantIngest.handleMsgCreateOrUpdateParticipantSession', {
              data: payload,
            })
            break
          case VeranaParticipantMessageTypes.SlashParticipantTrustDeposit:
            result = await this.broker.call('participantIngest.handleMsgSlashParticipantTrustDeposit', {
              data: payload,
            })
            break
          case VeranaParticipantMessageTypes.RepayParticipantSlashedTrustDeposit:
            result = await this.broker.call('participantIngest.handleMsgRepayParticipantSlashedTrustDeposit', {
              data: payload,
            })
            break
          default:
    ...
    this.logger.info(
      ` Participant processing complete: ${totalMessages - failedMessages.length}/${totalMessages} successful`
    )
    return { success: true, failed: failedMessages.length }
  }

  @Action({ name: 'getParticipant' })
  async getParticipant(ctx: Context<{ schema_id: number; corporation: string; role: string }>) {
  ...
  async listParticipants(ctx: Context<{ schema_id?: number; corporation?: string; role?: string }>) {
    const { schema_id: schemaId, corporation, role } = ctx.params
