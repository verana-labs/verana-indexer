...
import {
  ...
  syncEcosystemStatsAndHistoryFromSchemaChange,
} from '../crawl-cs/cs_database.service'
...
function computeChanges(oldRecord: any, newRecord: any, fields: string[]): Record<string, any> | null {
...
function extractParticipantType(msg: Record<string, any>, fallback: string | number = 'UNSPECIFIED') {
  return mapParticipantType(msg.participant_type ?? msg.participantType ?? msg.role ?? fallback)
}

function normalizeValidationState(value: unknown): string {
  if (typeof value === 'string') {
    const normalized = value.toUpperCase()
    if (normalized === 'VALIDATION_STATE_UNSPECIFIED' || normalized === 'PENDING' || normalized === 'VALIDATED') {
      return normalized
    }
...
function _jsonbColumnValue(value: unknown): string | object | null {
...
async function insertParticipantsWithSequenceHeal<T = any>(trx: any, insertData: any): Promise<T> {
...
async function pickParticipantSnapshot(record: any) {
  ...
  const hasParticipantRoleColumns = (
    await Promise.all(PARTICIPANT_ROLE_HISTORY_FIELDS.map((field) => checkParticipantHistoryColumnExists(field)))
  ).every(Boolean)
  ...
  for (const field of PARTICIPANT_HISTORY_FIELDS) {
    ...
    if (
      (field === 'ecosystem_slash_events' ||
        field === 'ecosystem_slashed_amount' ||
        field === 'ecosystem_slashed_amount_repaid' ||
        field === 'network_slash_events' ||
        field === 'network_slashed_amount' ||
        field === 'network_slashed_amount_repaid') &&
...
async function recordParticipantHistory(
  ...
  if (participantId != null && db && typeof db === 'function') {
    const fresh = await db('participants')
...
async function recordParticipantSessionHistory(
  ...
  const changes = computeChanges(previousRecord, sessionRecord, PARTICIPANT_SESSION_HISTORY_FIELDS)
...
export default class ParticipantIngestService extends Service {
    ...
    this.parseServiceSchema({
      ...
      actions: {
        handleMsgCreateRootParticipant: {
          params: { data: 'object' },
          handler: async (ctx) => this.handleCreateRootParticipant(ctx.params.data),
        },
        ...
        handleMsgSetParticipantOPToValidated: {
          params: { data: 'object' },
          handler: async (ctx) => this.handleSetParticipantOPToValidated(ctx.params.data),
        },
        ...
        handleMsgCancelParticipantOPLastRequest: {
          params: { data: 'object' },
          handler: async (ctx) => this.handleCancelParticipantOPLastRequest(ctx.params.data),
        },
        handleMsgCreateOrUpdateParticipantSession: {
          params: { data: 'object' },
          handler: async (ctx) => this.handleCreateOrUpdateParticipantSession(ctx.params.data),
        },
        handleMsgSlashParticipantTrustDeposit: {
          params: { data: 'object' },
          handler: async (ctx) => this.handleSlashParticipantTrustDeposit(ctx.params.data),
        },
        handleMsgRepayParticipantSlashedTrustDeposit: {
          params: { data: 'object' },
          handler: async (ctx) => this.handleRepayParticipantSlashedTrustDeposit(ctx.params.data),
        },
        syncParticipantFromLedger: {
          ...
          handler: async (ctx) =>
            this.syncParticipantFromLedger(
        ...
        syncParticipantSessionFromLedger: {
          ...
          handler: async (ctx) =>
            this.syncParticipantSessionFromLedger(
        ...
        compareParticipantWithLedger: {
          ...
          handler: async (ctx) =>
            this.compareParticipantWithLedger(
        ...
        compareParticipantSessionWithLedger: {
          ...
          handler: async (ctx) =>
            this.compareParticipantSessionWithLedger(
        ...
        listParticipants: {
          ...
          handler: async (ctx) => {
            ...
            if (ctx.params.schema_id) query = query.where('schema_id', ctx.params.schema_id)
  ...
  private async handleSetParticipantEffectiveUntil(msg: MsgSetParticipantEffectiveUntil & { height?: number }) {
    ...
    const participantId =
      Number((msg as any)?.id ?? (msg as any)?.participant_id ?? (msg as any)?.participantId ?? 0) || 0
    ...
    try {
      ...
      const _adjustedBy = extractController(msg as unknown as Record<string, unknown>) ?? null
  ...
  private async refreshEcosystemStatsBySchemaId(
    ...
    try {
      const cs = await knex('credential_schemas').where({ id: schemaId }).select('ecosystem_id').first()
      ...
      this.logger.warn(`Failed to refresh ecosystem stats/history for schema_id=${schemaId}: ${err?.message || err}`)
    }
  }
  ...
  private mapLedgerParticipantToDbRow(ledgerParticipant: Record<string, any>) {
    ...
    return {
      ...
      validator_participant_id:
        Number(ledgerParticipant.validator_participant_id ?? ledgerParticipant.validatorParticipantId ?? 0) || null,
      ...
      op_summary_digest: ledgerParticipant.op_summary_digest ?? ledgerParticipant.opSummaryDigest ?? null,
  ...
  private async syncParticipantFromLedger(
    ...
    await knex.transaction(async (trx) => {
      ...
      if (previousParticipant) {
        await trx('participants').where({ id: mapped.id }).update(payload)
      ...
      try {
        const hasFlipMetaColumns = await this.checkParticipantsColumnExists('last_valid_flip_version')
        const hasIsActiveNowColumn = await this.checkParticipantsColumnExists('is_active_now')
        ...
        if (hasFlipMetaColumns && finalParticipant) {
          ...
          if (stateInputsChanged) {
            ...
            const prevIsActiveNow =
              hasIsActiveNowColumn && previousParticipant ? Boolean((previousParticipant as any).is_active_now) : false

            const participantUpdate: Record<string, any> = {
              last_valid_flip_version: newVersion,
            }

            await trx('participants').where({ id: finalParticipant.id }).update(participantUpdate)

            const enterFlips: Array<{ flip_at_time: string; flip_kind: number }> = []
            const exitFlips: Array<{ flip_at_time: string; flip_kind: number }> = []

            const effectiveFrom = finalParticipant.effective_from ? new Date(finalParticipant.effective_from) : null
            const effectiveUntil = finalParticipant.effective_until ? new Date(finalParticipant.effective_until) : null
      ...
      try {
        ...
        if (slashDelta > BigInt(0) || repayDelta > BigInt(0)) {
          ...
          if (isEcosystemParticipant) {
            ...
            const schema = await trx('credential_schemas').where({ id: participantRow.schema_id }).first()
  ...
  private async syncParticipantSessionFromLedger(
    ...
    const enrichedAuthz = await Promise.all(
      recordsRaw.map(async (entry: any) => {
        ...
        if (directIssuer != null || directVerifier != null) {
          const walletAgentParticipantId =
            Number(
              entry?.wallet_agent_participant_id ??
                entry?.walletAgentParticipantId ??
                ledgerSession?.wallet_agent_participant_id ??
                0
            ) || 0
          return {
        ...
        const walletAgentParticipantId =
          Number(entry?.wallet_agent_participant_id ?? ledgerSession?.wallet_agent_participant_id ?? 0) || 0
        ...
        if (beneficiaryParticipantId > 0) {
          const beneficiaryParticipant = await knex('participants')
            .where({ id: beneficiaryParticipantId })
            .select('role')
            .first()
    ...
    const mappedSession: any = {
      id,
      corporation_id: Number(ledgerSession?.corporation_id ?? ledgerSession?.corporationId ?? 0) || 0,
      ...
      wallet_agent_participant_id:
        Number(ledgerSession?.wallet_agent_participant_id ?? ledgerSession?.walletAgentParticipantId ?? 0) || 0,
      session_records: JSON.stringify(
        enrichedAuthz.map((e: any, i: number) => ({
          created:
            toIsoOrNull(recordsRaw[i]?.created) ?? toIsoOrNull(ledgerSession?.modified) ?? new Date().toISOString(),
    ...
    await knex.transaction(async (trx) => {
      ...
      if (previous) {
        await trx('participant_sessions').where({ id }).update(mappedSession)
      ...
      for (const issuerId of newIssuerParticipantIds) {
        if (!previousIssuerParticipantIds.has(issuerId)) {
          try {
            ...
            try {
              await recordParticipantHistory(trx, { id: Number(issuerId) }, 'CREDENTIAL_ISSUED', effectiveHeight)
            } catch (historyErr: any) {
              this.logger.warn(
      ...
      for (const verifierId of newVerifierParticipantIds) {
        if (!previousVerifierParticipantIds.has(verifierId)) {
          try {
            ...
            try {
              await recordParticipantHistory(trx, { id: Number(verifierId) }, 'CREDENTIAL_VERIFIED', effectiveHeight)
            } catch (historyErr: any) {
              this.logger.warn(
      ...
      await knex.transaction(async (trx) => {
        ...
        for (const { id } of participants) {
          ...
          try {
            ...
            this.logger.warn(
              `[rebuildParticipantStats] Failed to update weight for participant ${pid}:`,
              err?.message || err
            )
          }
          try {
            ...
            this.logger.warn(
              `[rebuildParticipantStats] Failed to update participants for participant ${pid}:`,
              err?.message || err
            )
          }
        }
        ...
        for (const { id } of participants) {
          ...
          try {
            await trx('participants').where({ id: pid }).update({
              issued,
              verified,
  ...
  private normalizeComparableParticipantRecord(
    ...
    return {
      ...
      op_summary_digest: record.op_summary_digest ?? record.opSummaryDigest ?? null,
    }
  }

  private normalizeComparableParticipantSessionRecord(
    record: Record<string, any> | null | undefined
  ): Record<string, any> | null {
    if (!record) return null
    ...
    return {
      ...
      wallet_agent_participant_id:
        Number(record.wallet_agent_participant_id ?? record.walletAgentParticipantId ?? 0) || 0,
  ...
  private async handleCreateRootParticipant(msg: MsgCreateRootParticipant & { height?: number }) {
    ...
    try {
      ...
      if (!schemaId) {
        this.logger.error(
          'CRITICAL: Missing schema_id in MsgCreateRootParticipant, cannot create root participant. Msg keys:',
          Object.keys(msg)
        )
        return
      }
      ...
      this.logger.info(
        `[handleCreateRootParticipant] expire_soon calculated: ${expireSoon}, column exists: ${hasExpireSoonColumn}`
      )
      ...
      if (hasExpireSoonColumn) {
        ...
        this.logger.warn(
          `[handleCreateRootParticipant] expire_soon column does not exist, skipping. Please run database migrations.`
        )
      }
      ...
      if (!insertedParticipant) {
        this.logger.error('CRITICAL: Failed to create root participant - insert returned no record')
        return
      }
      ...
      try {
        ...
        this.logger.warn(
          `Failed to update participants for root participant ${participant.id}:`,
          participantsErr?.message || participantsErr
        )
      }

      try {
        await recordParticipantHistory(knex, participant, 'CREATE_ROOT_PARTICIPANT', height)
      } catch (historyErr: any) {
        this.logger.error('CRITICAL: Failed to record participant history for root participant:', historyErr)
      }
  ...
  private async handleCreateParticipant(msg: MsgSelfCreateParticipant & { height?: number }) {
    try {
      ...
      const explicitValidatorParticipantId =
        (msg as any).validatorParticipantId ??
        (msg as any).validator_participant_id ??
        (msg as any).validatorParticipantId ??
        (msg as any).validator_participant_id ??
        null
      ...
      if (!schemaId) {
        this.logger.warn(
          'Missing schema_id and could not infer it from validator_participant_id in MsgSelfCreateParticipant, skipping insert. Msg keys:',
          Object.keys(msg)
        )
        return
      }
      ...
      const ecosystemParticipant = await knex('participants').where({ schema_id: schemaId, role: 'ECOSYSTEM' }).first()

      if (!ecosystemParticipant) {
        this.logger.warn(`No root ECOSYSTEM participant found for schema_id=${schemaId}, cannot create ${role}`)
      }
      ...
      this.logger.info(
        `[handleCreateParticipant] expire_soon calculated: ${expireSoon}, column exists: ${hasExpireSoonColumn}`
      )
      ...
      if (hasExpireSoonColumn) {
        ...
        this.logger.warn(
          `[handleCreateParticipant] expire_soon column does not exist, skipping. Please run database migrations.`
        )
      }
      ...
      if (!participant) {
        this.logger.error('CRITICAL: Failed to create participant - insert returned no record')
        return
      }
      ...
      try {
        ...
        this.logger.warn(
          `Failed to update participants for participant ${participant.id}:`,
          participantsErr?.message || participantsErr
        )
      }

      try {
        await recordParticipantHistory(knex, participant, 'CREATE_PARTICIPANT', height)
      } catch (historyErr: any) {
        this.logger.error('CRITICAL: Failed to record participant history:', historyErr)
      }
  ...
  private async handleRevokeParticipant(msg: MsgRevokeParticipant & { height?: number }) {
    try {
      ...
      const applicantParticipant = await knex('participants').where({ id: msg.id }).first()
      ...
      if (callerCorpId !== null && applicantParticipant.validator_participant_id) {
        ...
        while (validatorParticipantId) {
          const validatorParticipant = await knex('participants').where({ id: validatorParticipantId }).first()
      ...
      if (!authorized && callerCorpId !== null) {
        const cs = await knex('credential_schemas').where({ id: applicantParticipant.schema_id }).first()
        if (cs) {
          const ec = await knex('ecosystem').where({ id: cs.ecosystem_id }).first()
      ...
      await knex.transaction(async (trx) => {
        ...
        const [updated] = await trx('participants').where({ id: msg.id }).update(updateData).returning('*')

        if (!updated) {
          this.logger.error(`CRITICAL: Failed to revoke participant ${msg.id} - update returned no record`)
        }
        ...
        try {
          ...
          this.logger.warn(
            `Failed to update participants for participant ${msg.id}:`,
            participantsErr?.message || participantsErr
          )
        }

        try {
          await recordParticipantHistory(trx, updated, 'REVOKE_PARTICIPANT', height, applicantParticipant)
        } catch (historyErr: any) {
          this.logger.error('CRITICAL: Failed to record participant history for revoke:', historyErr)
        }
      })

      await this.refreshEcosystemStatsBySchemaId(applicantParticipant?.schema_id, height)

      this.logger.info(`Participant ${msg.id} successfully revoked by ${caller}`)
      return { success: true }
    } catch (err: any) {
  ...
  private async handleStartParticipantOP(msg: MsgStartParticipantOP & { height?: number }) {
    try {
      ...
      const participant = await knex('participants').where({ id: validatorParticipantId }).first()

      if (!participant) {
        this.logger.warn(`Participant ${validatorParticipantId} not found, skipping OP start`)
        return
      }
      ...
      if (hasExpireSoonColumn) {
        ...
        this.logger.warn(
          `[handleStartParticipantOP] expire_soon column does not exist, skipping. Please run database migrations.`
        )
      }
      ...
      if (!newParticipant) {
        this.logger.error('CRITICAL: Failed to create participant via OP start - insert returned no record')
      }

      this.logger.info(`Inserted new OP entry handleStartParticipantOP: ${JSON.stringify(Entry)}`)
      ...
      try {
        ...
        this.logger.warn(
          `Failed to update weight/participants for new participant ${newParticipant.id}:`,
          updateErr?.message || updateErr
        )
      }

      try {
        await recordParticipantHistory(knex, newParticipant, 'START_PARTICIPANT_OP', height)
      } catch (historyErr: any) {
        this.logger.error('CRITICAL: Failed to record participant history for OP start:', historyErr)
      }
  ...
  public async computeOpExp(participant: any, knex: any): Promise<string | null> {
    const cs = await knex('credential_schemas').where({ id: participant.schema_id }).first()

    if (!cs) {
      const schemaExists = await knex('credential_schemas').where({ id: participant.schema_id }).first()

      if (!schemaExists) {
        this.logger.warn(
          `CredentialSchema ${participant.schema_id} not found for participant ${participant.id} - schema may not exist yet, queuing for retry`
        )
      } else {
        this.logger.warn(
          `CredentialSchema ${participant.schema_id} exists but not visible in current transaction for participant ${participant.id} - queuing for retry`
        )
      }
      return null
    }
    ...
    switch (participant.role) {
      ...
      default:
        this.logger.warn(
          `Unknown participant type '${participant.role}' for participant ${participant.id} - cannot compute op_exp`
        )
        return null
    }
    ...
    if (!participant.op_exp) {
      ...
      opExp = new Date(new Date(participant.op_exp).getTime() + validitySeconds * 1000)
    }

    return opExp.toISOString()
  }

  private async handleSetParticipantOPToValidated(msg: MsgSetParticipantOPToValidated & { height?: number }) {
    try {
      const now = formatTimestamp(msg.timestamp)
      const height = Number((msg as any)?.height) || 0
      ...
      if (participant.op_state !== 'PENDING') {
        this.logger.warn(
          `[SetOPToValidated] Participant ${msg.id} is not PENDING (current state: ${participant.op_state})`
        )
      ...
      if (msg.validation_fees < 0 || msg.issuance_fees < 0 || msg.verification_fees < 0) {
      ...
      if (!schemaExists) {
        ...
        this.logger.info(
          `Participant ${msg.id} queued for retry - waiting for CredentialSchema ${participant.schema_id}`
        )
      ...
      if (opExp === null) {
        const validityPeriodFieldName =
          participant.role === 'ISSUER_GRANTOR'
            ? 'issuer_grantor_validation_validity_period'
            : participant.role === 'VERIFIER_GRANTOR'
      ...
      const effectiveUntil = msg.effective_until ?? participant.effective_until ?? opExp ?? null

      if (effectiveUntil && opExp && new Date(effectiveUntil) > new Date(opExp)) {
        this.logger.warn(`effective_until ${effectiveUntil} exceeds op_exp ${opExp}`)
      ...
      if (isFirstValidation) {
        ...
        const feesChanged =
          (msg.validation_fees && msg.validation_fees !== participant.validation_fees) ||
          (msg.issuance_fees && msg.issuance_fees !== participant.issuance_fees) ||
          (msg.verification_fees && msg.verification_fees !== participant.verification_fees)
      ...
      const [updated] = await knex('participants').where({ id: msg.id }).update(entry).returning('*')
      ...
      try {
        await recordParticipantHistory(knex, updated, 'SET_VALIDATE_PARTICIPANT_OP', height, participant)
      } catch (historyErr: any) {
        this.logger.error('CRITICAL: Failed to record participant history for OP validation:', historyErr)
      }
  ...
  private async handleRenewParticipantOP(msg: MsgRenewParticipantOP) {
    try {
      ...
      if (!validatorParticipant) {
        this.logger.warn(`Validator participant ${applicantParticipant.validator_participant_id} not found`)
      ...
      await knex.transaction(async (trx) => {
        ...
        try {
          ...
          this.logger.warn(
            `Failed to update weight for participant ${msg.id} after OP renewal:`,
            weightErr?.message || weightErr
          )
        }
        ...
        try {
          await recordParticipantHistory(trx, updated, 'RENEW_PARTICIPANT_OP', height, applicantParticipant)
        } catch (historyErr: any) {
          this.logger.error('CRITICAL: Failed to record participant history for OP renewal:', historyErr)
        }
      })
  ...
  private async handleCancelParticipantOPLastRequest(msg: MsgCancelParticipantOPLastRequest & { height?: number }) {
    try {
      const now = formatTimestamp(msg.timestamp)
      const height = Number((msg as any)?.height) || 0
      ...
      const opValidatorDeposit = participant.op_validator_deposit
      ...
      const [updated] = await knex('participants').where({ id: msg.id }).update(updateData).returning('*')

      if (!updated) {
        this.logger.error(`CRITICAL: Failed to update participant ${msg.id} - update returned no record`)
      }
      ...
      try {
        await recordParticipantHistory(knex, updated, 'CANCEL_PARTICIPANT_OP', height, participant)
      } catch (historyErr: any) {
        this.logger.error('CRITICAL: Failed to record participant history for OP cancellation:', historyErr)
      }

      this.logger.info(`Participant ${msg.id} validation cancelled. New state: ${newOpState}`)
  ...
  private async handleSlashParticipantTrustDeposit(msg: MsgSlashParticipantTrustDeposit & { height?: number }) {
    try {
      ...
      if (!isAuthorized && callerCorpId !== null && participant.schema_id) {
        const schema = await knex('credential_schemas').where({ id: participant.schema_id }).first()
      ...
      if (isEcosystemParticipant) {
        ...
        if (!schema) {
          this.logger.warn(
            `[Slash] Participant ${msg.id} has schema_id ${participant.schema_id} but schema not found in database`
          )
          ...
          if (!ec) {
            this.logger.warn(
              `[Slash] Participant ${msg.id} schema ${participant.schema_id} references EC ${schema.ecosystem_id} but EC not found in database`
            )
            ...
            if (!ecCorpId) {
              this.logger.warn(
                `[Slash] Participant ${msg.id} EC ${schema.ecosystem_id} exists but has no corporation_id`
              )
              ...
              this.logger.info(
                `[Slash] Participant ${msg.id} slashed by EC corporation_id ${ecCorpId} - marking as ecosystem slash`
              )
            } else {
              this.logger.warn(
                `[Slash] Participant ${msg.id} slashed by ${caller} (corporation_id ${callerCorpId ?? 'none'}) but EC corporation_id is ${ecCorpId} - no slash type determined`
              )
      ...
      if (!isEcosystemSlash && !isNetworkSlash) {
        this.logger.error(
          `[Slash] CRITICAL: Could not classify slash for participant ${msg.id}. Classification reason: ${classificationReason}. Schema ID: ${participant.schema_id || 'N/A'}, Type: ${participant.role}, Caller: ${caller}, EC Controller: ${trController || 'N/A'}`
        )
      }

      await knex.transaction(async (trx) => {
        const currentDeposit = BigInt(participant.deposit || '0')
        const newDeposit = currentDeposit > BigInt(amountNum) ? currentDeposit - BigInt(amountNum) : BigInt(0)
        ...
        if (!updated) {
          this.logger.error(`CRITICAL: Failed to slash participant ${msg.id} - update returned no record`)
        }
        ...
        try {
          if (isEcosystemSlash || isNetworkSlash) {
            ...
            this.logger.info(
              `[Slash] Updated slash statistics for participant ${msg.id} - ecosystem: ${isEcosystemSlash}, network: ${isNetworkSlash}, amount: ${amountNum}`
            )
          } else {
            this.logger.warn(
              `[Slash] Skipping slash statistics update for participant ${msg.id} - neither ecosystem nor network slash detected`
            )
          }
        } catch (statsErr: any) {
        ...
        try {
          ...
          this.logger.warn(
            `Failed to update weight for participant ${Number(msg.id)}:`,
            weightErr?.message || weightErr
          )
        }
        ...
        try {
          await recordParticipantHistory(trx, updated, 'SLASH_PARTICIPANT_TRUST_DEPOSIT', height, participant)
        } catch (historyErr: any) {
          this.logger.error('CRITICAL: Failed to record participant history for slash:', historyErr)
          process.exit(1)
        }
      })
      ...
      this.logger.info(`✅ Participant ${msg.id} slashed by ${caller} amount ${amountNum}`)
  ...
  private async handleRepayParticipantSlashedTrustDeposit(
    ...
    try {
      ...
      if (slashedDeposit <= 0) {
        this.logger.warn(`Participant ${msg.id} has no slashed deposit to repay`)
      ...
      await knex.transaction(async (trx) => {
        ...
        const repaidAmount =
          requestedRepayAmount == null ? BigInt(slashedDeposit) : BigInt(String(requestedRepayAmount))
        ...
        if (!updated) {
          this.logger.error(`CRITICAL: Failed to repay participant ${msg.id} - update returned no record`)
        }
        ...
        try {
          ...
          this.logger.warn(
            `Failed to update weight for participant ${msg.id} after repay:`,
            weightErr?.message || weightErr
          )
        }
        ...
        try {
          await recordParticipantHistory(trx, updated, 'REPAY_PARTICIPANT_SLASHED_TRUST_DEPOSIT', height, participant)
        } catch (historyErr: any) {
          this.logger.error('CRITICAL: Failed to record participant history for repay:', historyErr)
        }
      })
      ...
      this.logger.info(`✅ Participant ${msg.id} slashed deposit (${slashedDeposit}) repaid by ${repayer}`)

      return { success: true }
    } catch (err: any) {
      this.logger.error('CRITICAL: Error in handleRepayParticipantSlashedTrustDeposit:', err)
  ...
  private async incrementParticipantStatistics(
    ...
    if (!hasIssuedColumn && !hasVerifiedColumn) {
      this.logger.warn(
        `[incrementParticipantStatistics] Neither issued nor verified column exists for participant ${participantId}`
      )
      return
    }
    ...
    if (incrementIssued && !hasIssuedColumn) {
      this.logger.warn(
        `[incrementParticipantStatistics] Attempted to increment issued for participant ${participantId} but issued column does not exist`
      )
    }

    if (incrementVerified && !hasVerifiedColumn) {
      this.logger.warn(
        `[incrementParticipantStatistics] Attempted to increment verified for participant ${participantId} but verified column does not exist`
      )
    }

    const initialParticipant: { schema_id: number; validator_participant_id: number | null } | undefined = await trx(
      'participants'
    )
      .where({ id: participantId })
      .select('schema_id', 'validator_participant_id')
      .first()
    ...
    while (currentParticipantId) {
      const participant: { schema_id: number; validator_participant_id: number | null } | undefined = await trx(
        'participants'
      )
      ...
      if (participant.schema_id !== schemaId) {
        this.logger.warn(
          `Participant tree traversal crossed schema boundary. participantId=${currentParticipantId}, expected schema=${schemaId}, found schema=${participant.schema_id}. Stopping traversal.`
        )
        break
      }
      ...
      if (Object.keys(updates).length > 0) {
        try {
          await trx('participants').where({ id: currentParticipantId }).update(updates)
        } catch (error: any) {
  ...
  private async updateSlashStatistics(
    ...
    if (!hasEcosystemSlashEventsColumn) {
      this.logger.warn(
        `[updateSlashStatistics] Column ecosystem_slash_events does not exist, skipping update for participant ${participantId}`
      )
      return
    }
    ...
    if (!isEcosystemSlash && !isNetworkSlash) {
      this.logger.warn(
        `[updateSlashStatistics] Neither ecosystem nor network slash flag is set for participant ${participantId}, skipping update`
      )
      return
    }
    ...
    if (isEcosystemSlash) {
      if (slashAmount !== 0) {
        ...
        updates.ecosystem_slashed_amount = knex.raw('COALESCE(ecosystem_slashed_amount, 0::numeric) + ?::numeric', [
          slashAmount,
        ])
      }
      if (repayAmount) {
    ...
    if (isNetworkSlash) {
      if (slashAmount !== 0) {
        ...
        updates.network_slashed_amount = knex.raw('COALESCE(network_slashed_amount, 0::numeric) + ?::numeric', [
          slashAmount,
        ])
      }
      if (repayAmount) {
    ...
    if (Object.keys(updates).length > 0) {
      try {
        const result = await trx('participants').where({ id: participantId }).update(updates)
        this.logger.debug(
  ...
  private async updateWeight(trx: any, participantId: number): Promise<void> {
    ...
    const initialParticipant:
      | { schema_id: number; validator_participant_id: number | null; deposit: number }
      | undefined = await trx('participants')
      .where({ id: participantId })
      .select('schema_id', 'validator_participant_id', 'deposit')
      .first()
    if (!initialParticipant) return
  ...
  private async updateParticipants(trx: any, participantId: number): Promise<void> {
    ...
    for (let i = participantStack.length - 1; i >= 0; i--) {
      ...
      try {
        ...
        await trx('participants').where({ id: pid }).update(updates)
      } catch (error: any) {
        if (error?.nativeError?.code === '42703') {
          this.participantsColumnExistsCache = null
          return
  ...
  private async handleCreateOrUpdateParticipantSession(msg: MsgCreateOrUpdateParticipantSession & { height?: number }) {
    ...
    try {
      ...
      if (!issuerParticipantId && !verifierParticipantId) {
        throw new Error('At least one of issuer_participant_id or verifier_participant_id must be provided')
      }

      const [agentParticipant, walletAgentParticipant, issuerParticipant, verifierParticipant] = await Promise.all([
        trx('participants').where({ id: agentParticipantId }).first(),
        trx('participants').where({ id: walletAgentParticipantId }).first(),
        issuerParticipantId ? trx('participants').where({ id: issuerParticipantId }).first() : null,
        verifierParticipantId ? trx('participants').where({ id: verifierParticipantId }).first() : null,
      ])
      ...
      const existing = await trx('participant_sessions').where({ id: msg.id }).first()
      ...
      if (!existing) {
        ...
        await recordParticipantSessionHistory(trx, normalizedSession, 'CREATE_PARTICIPANT_SESSION', height)
        ...
        if (issuerParticipantId) {
          try {
            ...
            try {
              ...
              this.logger.warn(
                `[Session] Failed to record participant history for issued participant ${issuerParticipantId}:`,
                historyErr?.message || historyErr
              )
            }
          } catch (issuedErr: any) {
            this.logger.error(
              `[Session] Failed to increment issued for participant ${issuerParticipantId}:`,
              issuedErr?.message || issuedErr
            )
          }
        }
        if (verifierParticipantId) {
          try {
            ...
            try {
              ...
              this.logger.warn(
                `[Session] Failed to record participant history for verified participant ${verifierParticipantId}:`,
                historyErr?.message || historyErr
              )
            }
          } catch (verifiedErr: any) {
            this.logger.error(
              `[Session] Failed to increment verified for participant ${verifierParticipantId}:`,
              verifiedErr?.message || verifiedErr
            )
          }
        }
          ...
          if (!previousIssuerParticipantIds.has(issuerId)) {
            try {
              ...
              try {
                ...
                this.logger.warn(
                  `[Session] Failed to record participant history for issued participant ${issuerId}:`,
                  historyErr?.message || historyErr
                )
              }
            } catch (issuedErr: any) {
              this.logger.error(
                `[Session] Failed to increment issued for participant ${issuerId}:`,
                issuedErr?.message || issuedErr
              )
            }
          }
          ...
          if (!previousVerifierParticipantIds.has(verifierId)) {
            try {
              ...
              try {
                ...
                this.logger.warn(
                  `[Session] Failed to record participant history for verified participant ${verifierId}:`,
                  historyErr?.message || historyErr
                )
              }
            } catch (verifiedErr: any) {
              this.logger.error(
                `[Session] Failed to increment verified for participant ${verifierId}:`,
                verifiedErr?.message || verifiedErr
              )
            }
          }
  ...
  private async processRetryQueue(): Promise<void> {
    ...
    for (const key of keysToRetry) {
      ...
      if (!queuedParticipant) continue
      try {
        this.logger.info(
          `Retrying participant ${queuedParticipant.message.id} (attempt ${queuedParticipant.retryCount + 1}/${this.MAX_RETRY_ATTEMPTS})`
        )
        ...
        if (result && result.success !== false) {
          ...
          this.logger.warn(
            `Participant ${queuedParticipant.message.id} still failed, scheduled next retry in ${delay}ms`
          )
        }
      } catch (error) {
        queuedParticipant.retryCount++
    ...
    for (const [key, queuedParticipant] of this.retryQueue.entries()) {
      if (queuedParticipant.retryCount >= this.MAX_RETRY_ATTEMPTS) {
        this.retryQueue.delete(key)
        this.logger.error(
          `Participant ${queuedParticipant.message.id} exceeded max retries (${this.MAX_RETRY_ATTEMPTS}), giving up`
        )
      }
    }
