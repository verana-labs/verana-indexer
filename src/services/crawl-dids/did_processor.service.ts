...
export default class DidMessageProcessorService extends BullableService {
  ...
  async handleDidMessages(ctx: { params: { messages: DidMessageType[] } }) {
    ...
    for (const message of messages) {
      ...
      // ---------------- ADD ----------------
      if (
        [DidMessages.AddDid, DidMessages.AddDidLegacy].includes(
          message.type as DidMessages
        )
      ) {
        processedDID = {
      ...
      // ---------------- RENEW ----------------
        ...
        (message.type === DidMessages.RenewDid ||
          message.type === DidMessages.RenewDidLegacy) &&
        message.did
      ) {
        const renewDeposit = (await calculateDidDeposit(message?.years)) ?? "0";
        const existingDid: DidMessageTypes | null = await this.broker.call(
        ...
        if (existingDid) {
          ...
          const updatedDid: DidMessageTypes = {
            ...
            deposit: (
              parseInt(existingDid.deposit || "0") + parseInt(newDeposit)
            ).toString(),
            years: (parseInt(existingDid.years || "0") + yearsToAdd).toString(),
          };

          const changes = this.computeChanges(existingDid, updatedDid);
      ...
      // ---------------- TOUCH ----------------
        ...
        (message.type === DidMessages.TouchDid ||
          message.type === DidMessages.TouchDidLegacy) &&
        message.did
      ) {
        const existingDid: DidMessageTypes | null = await this.broker.call(
          `${SERVICE.V1.DidDatabaseService.path}.get`,
      ...
      // ---------------- REMOVE ----------------
        ...
        (message.type === DidMessages.RemoveDid ||
          message.type === DidMessages.RemoveDidLegacy) &&
        message.did
      ) {
        const existingDid: DidMessageTypes | null = await this.broker.call(
          `${SERVICE.V1.DidDatabaseService.path}.get`,
