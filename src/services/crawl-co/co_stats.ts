...
function mapTrustDepositRow(row: TrustDeposit): CorporationTrustDepositSnapshot {
  return {
...
export async function getCorporationTrustDeposit(address: string | null): Promise<CorporationTrustDepositSnapshot> {
  ...
  return row ? mapTrustDepositRow(row) : emptyTrustDepositSnapshot()
}
...
export async function getCorporationTrustDepositBatch(
  ...
  for (const row of rows) {
    result.set(row.corporation, mapTrustDepositRow(row))
  }
  return result
}
