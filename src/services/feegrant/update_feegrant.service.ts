...
export default class UpdateFeegrantService extends BullableService {
  ...
  async jobHandler(): Promise<void> {
    ...
    if (unprocessFeegrantHistories.length > 0) {
      this.logger.info(
        `handle unprocess feegrant event from ${unprocessFeegrantHistories[0].id} to ${
      ...
      await knex.transaction(async (trx) => {
        unprocessFeegrantHistories.forEach((e) => {
          ...
          const suspiciousFeegrants = originalFeegrants.filter(
            (x) => x.grantee === e.grantee && x.granter === e.granter && (x?.init_tx_id || 0) <= e.tx_id
          )
          if (suspiciousFeegrants.length > 0) {
            const originalFeegrant = suspiciousFeegrants.reduce((prev, current) =>
              (prev?.init_tx_id || 0) > (current?.init_tx_id || 0) ? prev : current
            )
  ...
  async updateFeegrant(feegrantHistories: FeegrantHistory[], trx: Knex.Transaction) {
    ...
    if (feegrantHistories.length > 0) {
        ...
        if (e.action !== FEEGRANT_ACTION.CREATE) {
          ...
          if (currentUpdate) {
            if (e.action === FEEGRANT_ACTION.USE) {
              if (e.amount) {
                const tmpAmount = (BigInt(currentUpdate.amount) + BigInt(e.amount)).toString()
      ...
      const originalFeegrants = await Feegrant.query().whereIn('id', Array.from(mapUpdate.keys()))
          ...
          if (e.spend_limit) {
            queriesUpdate.push(
              Feegrant.query()
                .where('id', e.id)
                .patch({
                  spend_limit: (BigInt(e.spend_limit) - BigInt(updateData.amount)).toString(),
      ...
      listRevoke.forEach((e) => {
        if (e.feegrant_id) {
          bulkUpdateOriginRevoke.push(
            Feegrant.query().where('id', e.feegrant_id).patch({ revoke_tx_id: e.tx_id }).transacting(trx)
          )
        } else {
          throw new Error(`Not found origin feegrant for fee grant event: ${e.id}`)
        }
      })
