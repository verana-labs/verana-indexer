...
import { BlockCheckpoint, Event, EventAttribute, Feegrant, FeegrantHistory } from '../../models'
...
export default class HandleFeegrantHistoryService extends BullableService {
  ...
  async jobHandler(): Promise<void> {
    ...
    const [startBlock, endBlock, updateBlockCheckpoint] = await BlockCheckpoint.getCheckpoint(
    ...
    const { newFeegrants, newHistories } = await this.getFeegrantEvents(startBlock, endBlock)
    await knex.transaction(async (trx) => {
      ...
      await BlockCheckpoint.query().insert(updateBlockCheckpoint).onConflict('job_name').merge().transacting(trx)
    })
  }
  ...
  async getFeegrantEvents(startBlock: number, endBlock: number) {
    ...
    const feegrantEvents = await Event.query()
      .alias('event')
      .withGraphJoined('[attributes(selectAttribute),transaction(selectTransaction),message(selectMessage)]')
      .modifiers({
      ...
      .whereIn('type', [Event.EVENT_TYPE.USE_FEEGRANT, Event.EVENT_TYPE.SET_FEEGRANT, Event.EVENT_TYPE.REVOKE_FEEGRANT])
    ...
    feegrantEvents.forEach((feegrantEvent) => {
      if (feegrantEvent.type === Event.EVENT_TYPE.USE_FEEGRANT) {
        if (feegrantEvent.transaction.fee) {
          const fee: { amount: string; denom: string } = feegrantEvent.transaction.fee[0]
          newHistories.push(
            FeegrantHistory.fromJson({
              tx_id: feegrantEvent.transaction.id,
              action: FEEGRANT_ACTION.USE,
              amount: fee.amount,
              granter: getAttributeFrom(feegrantEvent.attributes, EventAttribute.ATTRIBUTE_KEY.GRANTER),
              grantee: getAttributeFrom(feegrantEvent.attributes, EventAttribute.ATTRIBUTE_KEY.GRANTEE),
              denom: fee.denom,
            })
          )
        } else {
          throw new Error(`Transaction ${feegrantEvent.transaction.id} not found fee`)
        }
      }
    })
    feegrantEvents.forEach((feegrantEvent) => {
        ...
        if (feegrantEvent.tx_msg_index !== null) {
          ...
          const index = newHistories.findIndex(
            (e: FeegrantHistory) =>
              ...
              e.granter === getAttributeFrom(feegrantEvent.attributes, EventAttribute.ATTRIBUTE_KEY.GRANTER) &&
              e.grantee === getAttributeFrom(feegrantEvent.attributes, EventAttribute.ATTRIBUTE_KEY.GRANTEE)
          )
  ...
  getCreateFeegrantInfo(message: any): {
    ...
    return {
      ...
      denom,
      expiration: basicAllowance?.expiration ? new Date(basicAllowance.expiration) : undefined,
    }
  }
