...
export default class CrawlTrustDepositService extends BullableService {
  ...
  public async started() {
    try {
      ...
      const baseCrawlInterval =
        this._isFreshStart && config.crawlTrustDeposit.freshStart
      ...
      do {
        ...
        try {
          ...
          if (newHeight > lastHeight) {
            ...
            this.logger.info(
              `[CrawlTrustDepositService] Processed blocks up to height ${newHeight}, checkpoint updated`
            )
          }
  ...
  private async processBlockEventsInternal(block: Block | { height: number; block_result: any }) {
    ...
    const eventTypes = [TrustDepositEventType.AdjustTrustDeposit, TrustDepositEventType.SlashTrustDeposit]
  ...
  private async processTrustDepositHeightSync(height: number, events: any[]) {
    ...
    extractAccountAddressesFromEvents(events, true).forEach((a) => {
      allAccounts.add(a)
    })
    ...
    for (const depositId of toProcess) {
      try {
        ...
        await this.broker.call(`${SERVICE.V1.TrustDepositDatabaseService.path}.syncFromLedger`, {
  ...
  private async handleSlashTrustDepositEvent(height: number, event: any) {
    try {
      ...
      const result = await this.broker.call(`${SERVICE.V1.TrustDepositDatabaseService.path}.slash_trust_deposit`, {
        account,
