...
function computeTdChanges(oldRecord: any, newRecord: any, isCreation: boolean = false): Record<string, any> | null {
...
async function recordTrustDepositHistory(trx: any, td: any, eventType: string, height: number, previousRecord?: any) {
  if (!td) return
  ...
  const existingHistory = await trx('trust_deposit_history').where({ corporation: td.corporation, height }).first()
...
export default class TrustDepositMessageProcessorService extends BullableService {
  ...
  async started() {
    ...
    this.logger.info(
      `TrustDeposit message processor started | Mode: ${this._isFreshStart ? 'Fresh Start' : 'Reindexing'}`
    )
  ...
  private async loadTrustDepositParams() {
    try {
      const params = await ModuleParams.query().where('module', ModulesParamsNamesTypes.TD).first()
      this.trustDepositParams = params?.params || {}
      this.logger.info('[TrustDeposit] ✅ Loaded module params', this.trustDepositParams)
    } catch (error) {
  ...
  async handleTrustDepositMessages(ctx: any) {
    ...
    const params = this.trustDepositParams?.params || this.trustDepositParams || {}
    ...
    try {
      await knex.transaction(async (trx) => {
        ...
        await this.processorBase.processInBatches(trustDepositList, processMessageInTx, {
  ...
  private async reclaimYield(content: any, params: any, trx: any, height: number) {
    try {
      ...
      const claimableYield = toBigIntSafe(td.share) * shareValue - toBigIntSafe(td.deposit)
      ...
      await recordTrustDepositHistory(trx, updated, 'RECLAIM_YIELD', height, previousRecord)

      this.logger.info(`[TrustDeposit] ✅ Yield reclaimed for ${account}: ${claimableYield}`)
    } catch (error: any) {
      const errorMessage = error?.message || String(error)
      this.logger.error(`[TrustDeposit] ❌ Yield reclaim failed: ${errorMessage}`)
      throw error
    }
  }
  ...
  private async repaySlashed(content: any, ts: string, params: any, trx: any, height: number) {
    try {
      ...
      await recordTrustDepositHistory(trx, updated, 'REPAY_SLASHED', height, previousRecord)

      this.logger.info(`[TrustDeposit] ✅ Slashed deposit repaid ${amount} for ${account}`)
    } catch (error: any) {
      const errorMessage = error?.message || String(error)
      this.logger.error(`[TrustDeposit] ❌ Slashed deposit repay failed: ${errorMessage}`)
      throw error
    }
  }
  ...
  private async slashTrustDeposit(content: any, ts: string, height: number) {
    try {
      ...
      await this.broker.call(`${SERVICE.V1.TrustDepositDatabaseService.path}.slash_trust_deposit`, {
        account,
      ...
      const errorMessage = error?.message || String(error)
      this.logger.error(`[TrustDeposit] ❌ Slash failed: ${errorMessage}`)
      throw error
    }
  }
}
