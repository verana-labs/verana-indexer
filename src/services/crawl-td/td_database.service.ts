...
function computeTdChanges(oldRecord: any, newRecord: any): Record<string, any> | null {
...
function toBigIntSafe(value: any): bigint {
  ...
  if (!/^-?\d+$/.test(str)) {
    console.warn(`[TrustDeposit] ⚠️ Invalid BigInt value "${str}", defaulting to 0`)
    return BigInt(0)
  }
...
export default class TrustDepositDatabaseService extends BullableService {
  ...
  public async syncFromLedger(ctx: any) {
    ...
    const corporation = String(ledgerTrustDeposit?.corporation ?? ledgerTrustDeposit?.account ?? '').trim()
    if (!corporation) {
    ...
    try {
      await knex.transaction(async (trx) => {
        ...
        await recordTrustDepositHistory(trx, finalRecord, eventType || 'SYNC_LEDGER', height, previous)
      })

      return { success: true, corporation }
    } catch (error: any) {
      this.logger.warn(
        `[TrustDepositDB] syncFromLedger failed for corporation=${corporation}: ${error?.message || error}`
      )
    ...
    try {
      const result = await knex.transaction(async (trx) => {
        ...
        if (!existing) {
          ...
          await recordTrustDepositHistory(trx, inserted, 'CREATE_TRUST_DEPOSIT', blockHeight)
        ...
        await recordTrustDepositHistory(trx, updated, 'ADJUST_TRUST_DEPOSIT', blockHeight, previousRecord)
  ...
  public async slashTrustDeposit(ctx: any) {
    ...
    try {
      const result = await knex.transaction(async (trx) => {
        ...
        if (!td) {
          this.logger.warn(`[SlashTD] ❌ No trust deposit found for ${account}`)
          return {
            success: false,
        ...
        if (amountBig > currentAmount) {
          this.logger.warn(`[SlashTD] ❌ Slash amount ${amountBig} exceeds current deposit ${currentAmount}`)
          return { success: false, message: 'Slash amount exceeds deposit' }
        }

        const shareValue = toBigIntSafe(this.trustDepositParams?.params?.trust_deposit_share_value || '1')
        const now = formatTimestamp(lastSlashed)

        const newAmount = currentAmount - amountBig
        const newShare = toBigIntSafe(td.share) - (shareValue > BigInt(0) ? amountBig / shareValue : BigInt(0))
        ...
        const updated = await TrustDeposit.query(trx).patchAndFetchById(td.id, {
          ...
          slash_count: slashCount ? Number(slashCount) : Number(newSlashCount),
        })

        await recordTrustDepositHistory(trx, updated, 'SLASH_TRUST_DEPOSIT', blockHeight, previousRecord)

        this.logger.info(`[SlashTD] ⚔️ Global slash: ${account} slashed ${amountBig.toString()} at ${now}`)
  ...
  public async slashParticipantTrustDeposit(ctx: any) {
    ...
    try {
      const result = await knex.transaction(async (trx) => {
        ...
        if (!td) {
          this.logger.warn(`[SlashParticipantTD] ❌ No trust deposit found for ${corporation}`)
          return {
            success: false,
            message: 'TrustDeposit entry does not exist',
          }
        ...
        const currentAmount = toBigIntSafe(td.deposit)
        if (amountBig > currentAmount) {
          this.logger.warn(`[SlashParticipantTD] ❌ Slash amount ${amountBig} exceeds deposit ${currentAmount}`)
          return { success: false, message: 'Slash amount exceeds deposit' }
        }
        ...
        await recordTrustDepositHistory(trx, updated, 'SLASH_PARTICIPANT_TRUST_DEPOSIT', blockHeight, previousRecord)

        this.logger.info(`[SlashParticipantTD] ⚔️ Participant slash: ${corporation} slashed ${amountBig.toString()}`)
