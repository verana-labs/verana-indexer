...
import { buildVtChangesEnvelope, parseVtChangesQuery, type VtChange } from '../api/vt_subscribe_protocol'
import { buildVtChangesForBlock, listVtChangeHeights } from '../resolver/vt_change_detection'
import { decodeOffsetCursor, listIndexedDidsPage, parseIndexedDidsLimit } from '../resolver/vt_indexed_dids'
...
export default class IndexerMetaService extends BaseService {
  ...
  public async getBlockHeight(ctx: Context) {
    ...
    const updatedAt = checkpoint.updated_at instanceof Date ? checkpoint.updated_at : new Date(checkpoint.updated_at)
  ...
  public async listChanges(ctx: Context<unknown>) {
    ...
    const queryHistoryWithTx = async (
      ...
      try {
        const rows = await knex(tableName).select(`${tableName}.*`).where(`${tableName}.height`, height)
    ...
    const [timestampAtHeight, allMessagesAtHeight] = await Promise.all([heightTimestampPromise, heightMessagesPromise])
  ...
  public async listVtChanges(ctx: Context<Record<string, unknown>>) {
    ...
    while (blocks.length < limit) {
      ...
      for (let i = 0; i < heights.length; i++) {
        ...
        const envelope = buildVtChangesEnvelope(height, blockTime, raw, didFilter, corporationId, channels)
    ...
    return ApiResponder.success(ctx, { currentBlock, fromBlock, blocks, nextFromBlock }, 200)
  }
  ...
  private async blockTimesForHeights(heights: number[]): Promise<Map<number, string>> {
    ...
    const rows = (await knex('block').select('height', 'time').whereIn('height', heights)) as Array<{
      height: number
      time: Date | string
    }>
