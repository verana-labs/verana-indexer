...
import {
  PricingAssetType,
  pricingAssetTypeFromJSON,
} from "@verana-labs/verana-types/codec/verana/cs/v1/types";
...
export default class ExchangeRateApiService extends BaseService {
  ...
  async getPrice(ctx: Context<GetPriceParams>) {
    try {
      ...
      if (!active) {
        ...
        if (anyEntry) {
          return ApiResponder.error(
            ctx,
            "Exchange rate is disabled or expired",
            410
          );
        }
      ...
      return ApiResponder.error(
        ctx,
        `Failed to get price: ${err?.message || String(err)}`,
        500
      );
    }
  }
