...
import { BlockCheckpoint, Delegator, Transaction, TransactionMessage, Validator } from '../../models'
...
export default class CrawlDelegatorsService extends BullableService {
  ...
  public async updateAllValidator(_payload: Context<{ height: number }>): Promise<void> {
    await knex.raw(`TRUNCATE TABLE ${Delegator.tableName} RESTART IDENTITY CASCADE`)
    ...
    const latestTransactionMessage = await TransactionMessage.query()
      ...
      .orderBy('id', 'DESC')
      .limit(1)
    const blockCheckPoint = await BlockCheckpoint.query().findOne('job_name', BULL_JOB_NAME.CHECKPOINT_UPDATE_DELEGATOR)
  ...
  public async handleJobCrawlValidatorDelegators(_payload: IValidatorDelegators): Promise<void> {
    this.logger.info(`Update delegator for validator ${_payload.address}`)

    const delegators: Delegator[] = await this.getValidatorDelegations(_payload.address, _payload.id, _payload.height)
  ...
  public async getValidatorDelegations(
    ...
    for (;;) {
      const data = toHex(
        providerRegistry.cosmos.staking.v1beta1.QueryValidatorDelegationsRequest.encode(request).finish()
      )
  ...
  public async getCheckpointUpdateDelegator(): Promise<BlockCheckpoint> {
    ...
    if (!checkpointDelegator) {
      const oldestTransactionMessage = await TransactionMessage.query().orderBy('id', 'ASC').limit(1)
      ...
      await BlockCheckpoint.query().insert(checkpointDelegator).onConflict('job_name').ignore()
      checkpointDelegator =
  ...
  public async handleDelegateTxMsg(delegateTxMsg: TransactionMessage, trx: Knex.Transaction): Promise<void> {
    const validator = await Validator.query().findOne('operator_address', delegateTxMsg.content.validator_address)
    ...
    if (!delegator) {
      ...
      await trx(Delegator.tableName)
        .update({
          amount: BigNumber(delegator.amount).plus(delegateTxMsg.content.amount.amount).toString(),
        })
        .where({
          id: delegator.id,
        })
  ...
  public async handleReDelegateTxMsg(reDelegateTxMsg: TransactionMessage, trx: Knex.Transaction): Promise<void> {
    ...
    if (delegatorSrc) {
      const remainDelegateSrcAmount = BigNumber(delegatorSrc.amount).minus(reDelegateTxMsg.content.amount.amount)
    ...
    if (!delegatorDst) {
      ...
      await trx(Delegator.tableName)
        .update({
          amount: BigNumber(delegatorDst.amount).plus(reDelegateTxMsg.content.amount.amount).toString(),
        })
        .where({
          id: delegatorDst.id,
        })
  ...
  public async handleUnDelegateTxMsg(unDelegateTxMsg: TransactionMessage, trx: Knex.Transaction): Promise<void> {
    ...
    const remainDelegateAmount = BigNumber(delegator.amount).minus(unDelegateTxMsg.content.amount.amount)
  ...
  public async handleJob(): Promise<void> {
    ...
    const latestBlockCrawlValidator = await BlockCheckpoint.query().findOne('job_name', BULL_JOB_NAME.CRAWL_VALIDATOR)
