...
export default class RenameEventWithEventPartitionJob extends BullableService {
  ...
  public async switchEventPartitionToEvent(): Promise<void> {
    ...
    try {
      ...
      const currentEventIdSeq = await trx.raw('SELECT last_value FROM transaction_event_id_seq;')
      const lastEventId = currentEventIdSeq.rows[0].last_value
      await trx.raw(`SELECT setval('event_partition_id_seq', ${lastEventId}, true);`)
      await trx.commit()
  ...
  public async migrateEventPartition(): Promise<void> {
    ...
    if (latestEvent[0]?.id.toString() !== migrateEventCheckPoint?.height.toString()) {
      this.logger.info(`
