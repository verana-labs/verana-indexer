...
export default class CreateConstraintInAttrPartitionJob extends BullableService {
  ...
  public async addStatusToPartition(listPartition: any[]) {
    const handleTxCheckpoint = await BlockCheckpoint.query().findOne('job_name', BULL_JOB_NAME.HANDLE_TRANSACTION)
  ...
  public async createConstraintInPartition(_payload: {
    ...
    if (constraintResult.rows.length > 0 && regexAll.test(constraintResult.rows[0].definition)) {
      return
    }

    await knex.raw(`set statement_timeout to ${config.jobCreateConstraintInAttrPartition.statementTimeout}`)
    ...
    if (constraintResult.rows.length > 0 && regexLowerBoundOnly.test(constraintResult.rows[0].definition)) {
      ...
      if (_payload.status === this.statusPartition.done) {
        ...
        await knex.transaction(async (trx) => {
          await knex.raw(`alter table ${_payload.name} drop constraint ${currentContraintName}`).transacting(trx)
          await knex
            .raw(
              `alter table ${_payload.name} add constraint ${contraintName} check ${createFullConstraintScript} not valid`
            )
            .transacting(trx)
          await knex.raw(`alter table ${_payload.name} validate constraint ${contraintName}`).transacting(trx)
        })
        this.logger.info(`Dropped and created new constraint in partition ${_payload.name}`)
      }
    }
    ...
    if (constraintResult.rows.length === 0) {
      if (_payload.status === this.statusPartition.done) {
        ...
        await knex.raw(`alter table ${_payload.name} validate constraint ${contraintName}`)
        this.logger.info(`Created new full constraint in partition ${_payload.name}`)
        ...
        await knex.raw(`alter table ${_payload.name} validate constraint ${contraintName}`)
        this.logger.info(`Created new lower bound constraint in partition ${_payload.name}`)
      }
    }
  }
  ...
  public async _start(): Promise<void> {
    this.createJob(
      ...
      {},
      {
        ...
        repeat: {
          every: config.jobCreateConstraintInAttrPartition.jobRepeatCheckNeedCreateConstraint.millisecondRepeatJob,
        },
      }
    )
    return super._start()
