...
export default class UpdateSenderInTxMessages extends BullableService {
  ...
  async updateSender(_payload: { lastBlockCrawled: number }) {
    try {
      ...
      this.logger.info(`Update sender in transaction_message table start from block ${blockCheckpoint?.height}`)
      if (blockCheckpoint?.height === _payload.lastBlockCrawled) {
        return
      }

      const isReindexing = blockCheckpoint?.height === 0 || (blockCheckpoint?.height ?? 0) < 1000
      const jobConfig = config.jobUpdateSenderInTxMessages
      const effectiveConfig =
        isReindexing && jobConfig.reindexing ? { ...jobConfig, ...jobConfig.reindexing } : jobConfig

      let lastBlock = (blockCheckpoint?.height ?? 0) + effectiveConfig.blocksPerCall
      ...
      const listUpdates = listTx.map((tx) => {
        try {
          ...
          if (!isReindexing) {
            this.logger.debug(
              `Tx ${tx.hash} does not have message.sender attribute (this is normal for some transaction types)`
            )
          }
          return {
            tx_id: tx.id,
  ...
  private _findFirstAttribute(events: any, eventType: string, attributeKey: string): string {
    let result = ''
