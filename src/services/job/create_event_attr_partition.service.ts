...
export default class CreateEventAttrPartitionJob extends BullableService {
  ...
  async jobCreateEventAttrPartition(_payload: { startBlock: number; endBlock: number }) {
    this.logger.info(`Job create event attribute partition from ${_payload.startBlock} to ${_payload.endBlock}`)
    const tableName = `event_attribute_partition_${_payload.startBlock}_${_payload.endBlock}`
    await knex.transaction(async (trx) => {
      await knex.raw(`DROP TABLE IF EXISTS ${tableName}`).transacting(trx)
  ...
  async jobCheckIfNeedCreateEventAttrPartition() {
    const [blockHeightCrawledCheckpoint, eventAttrPartitionCheckpoint] = await Promise.all([
    ...
    if (blockHeightCrawledCheckpoint && eventAttrPartitionCheckpoint) {
      if (
        blockHeightCrawledCheckpoint.height + config.jobCheckNeedCreateEventAttributePartition.range >=
        ...
        this.createJob(
          ...
          BULL_JOB_NAME.JOB_CREATE_EVENT_ATTR_PARTITION,
          {
            ...
            endBlock: eventAttrPartitionCheckpoint.height + config.jobCheckNeedCreateEventAttributePartition.step,
          },
          {
  ...
  public async _start(): Promise<void> {
    this.createJob(
      ...
      {},
      {
        ...
        repeat: {
          every: config.jobCheckNeedCreateEventAttributePartition.millisecondCrawl,
        },
      }
    )
    return super._start()
