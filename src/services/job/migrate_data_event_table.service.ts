...
export default class MigrateDataEventTableJob extends BullableService {
  ...
  public async createPartitionForEventPartition(
    ...
    for (fromValue; fromValue < endValue; fromValue += partitionValueRange) {
      ...
      await knex.transaction(async (trx) => {
        await knex.raw(`CREATE TABLE ${partitionName} (LIKE event_partition INCLUDING ALL)`).transacting(trx)
        await knex
          .raw(
            `
  ...
  public async migrateDataFromEventToEventPartition(startId: number): Promise<void> {
    let done = false
    ...
    while (!done) {
      ...
      try {
        await trx.batchInsert('event_partition', events, config.migrationEventToPartition.chunkSizeInsert)
        await trx
  ...
  public async switchEventPartitionToEvent(): Promise<void> {
    await knex.transaction(async (trx) => {
      ...
      const currentEventIdSeq = await knex.raw('SELECT last_value FROM transaction_event_id_seq;')
      const lastEventId = currentEventIdSeq.rows[0].last_value
      await knex.raw(`SELECT setval('event_partition_id_seq', ${lastEventId}, true);`).transacting(trx)
    })
  }
  ...
  public async migrateEventPartition(): Promise<void> {
    ...
    if (!blockCheckpointMigrate) {
      ...
      await BlockCheckpoint.query().insert(newBlockCheckPoint).onConflict('job_name').ignore()
      currentCheckPointJob =
        (await BlockCheckpoint.query().where('job_name', BULL_JOB_NAME.CP_MIGRATE_DATA_EVENT_TABLE).first()) ??
        newBlockCheckPoint
    } else {
      currentCheckPointJob = blockCheckpointMigrate
    }

    const currentLatestEventId = await Event.query().orderBy('id', 'DESC').first()
    // .limit(1);
    ...
    await this.createPartitionForEventPartition(startId, endId, !!blockCheckpointMigrate)
    // Copy data from old table to new
