...
export default class CreateConstraintInEventPartitionJob extends BullableService {
  ...
  public async getLatestEventByPartition(partitionName: string): Promise<Event> {
  ...
  public async getMaxMinTxIdAndBlhByPartition(partitionName: string): Promise<{
    ...
    await knex.raw(`set statement_timeout to ${config.jobCreateConstraintInEventPartition.statementTimeout}`)
  ...
  public async getPartitionInsertionInfo(partitionName: string, toId: string): Promise<string> {
    const latestEventPartition = await this.getLatestEventByPartition(partitionName)

    if (!latestEventPartition) return this.insertionStatus.empty

    const endValueOfPartition = BigNumber(toId).minus(1)

    if (endValueOfPartition.eq(latestEventPartition.id)) return this.insertionStatus.done
  ...
  public async prepareConstrainInformation(
    ...
    const maxMinTxIdAndBlockHeight = await this.getMaxMinTxIdAndBlhByPartition(partitionName)
  ...
  public async createEventConstraint(_payload: { name: string; fromId: string; toId: string }): Promise<string> {
  ...
  public async createConstraintInEventPartition() {
    ...
    listPartition.forEach((partition: { name: string; fromId: string; toId: string }) => {
      this.createJob(BULL_JOB_NAME.JOB_CREATE_EVENT_CONSTRAIN, BULL_JOB_NAME.JOB_CREATE_EVENT_CONSTRAIN, partition, {
  ...
  public async _start(): Promise<void> {
    this.createJob(
      ...
      {},
      {
        ...
        repeat: {
          every: config.jobCreateConstraintInEventPartition.jobRepeatCheckNeedCreateConstraint.millisecondRepeatJob,
        },
      }
    )
    return super._start()
