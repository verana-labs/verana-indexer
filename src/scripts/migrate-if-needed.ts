...
;(async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Host: ${connInfo.host}, Port: ${connInfo.port}, User: ${connInfo.user}, Database: ${connInfo.database}`
    )
    ...
    try {
      ...
      if (migrationsTableExists) {
        try {
          ...
          if (
            ...
            errorMsg.includes('knex_migrations')
          ) {
      ...
      const shouldRunMigrations =
        !blockExists || !migrationsTableExists || (pending && pending.length > 0) || missingCriticalTables.length > 0
      ...
      if (!shouldRunMigrations) {
        ...
        if (missingCriticalTables.length > 0) {
          console.log(
            ` Found ${missingCriticalTables.length} missing critical table(s) (env: ${environment}): ${missingCriticalTables.join(', ')}`
          )
        ...
        if (blockExists && !txMsgExists) {
          ...
          await db.schema
            .createTable('transaction_message', (table) => {
              table.increments('id').primary()
        ...
        if (!trustResultsExists && migrationsTableExists) {
          try {
            const deleted = await db('knex_migrations').where('name', 'like', '%create_trust_results%').delete()
            if (deleted > 0) {
      ...
      if (errorMsg.includes('corrupt') || errorMsg.includes('missing')) {
        ...
        const criticalTables = ['account', 'transaction', 'ecosystem', 'credential_schemas', 'participants']
        ...
        if (missingCriticalTables.length > 0) {
          console.log(
            `  ❌ CRITICAL: ${missingCriticalTables.length} critical table(s) are missing: ${missingCriticalTables.join(', ')}`
          )
          ...
          try {
            ...
            try {
              ...
              for (const mig of allMigrations) {
                ...
                if (migName.includes('account') && missingCriticalTables.includes('account')) {
                  ...
                  migName.includes('credential_schema') &&
                  missingCriticalTables.includes('credential_schemas')
                ) {
                  migrationsToClear.push(migName)
              ...
              if (migrationsToClear.length > 0) {
                console.log(
                  `  Clearing ${migrationsToClear.length} additional migration record(s) that should create missing tables...`
                )
            ...
            if (stillMissing.length > 0) {
              console.error(
                `  ❌ CRITICAL ERROR: ${stillMissing.length} table(s) still missing after migration: ${stillMissing.join(', ')}`
              )
            ...
            if (
              forceErrorMsg.includes('corrupt') ||
              forceErrorMsg.includes('missing') ||
              forceErrorMsg.includes('files are missing')
            ) {
              ...
              try {
                ...
                if (missingFileMatch) {
                  ...
                  console.log(
                    `  Found ${missingFiles.length} migration(s) marked as missing: ${missingFiles.join(', ')}`
                  )
                  ...
                  for (const fileName of missingFiles) {
                    try {
                      ...
                      const isAlterMigration =
                        nameWithoutExt.includes('add_') ||
                      ...
                      if (isAlterMigration) {
                        ...
                        if (nameWithoutExt.includes('gfd_id') || nameWithoutExt.includes('document_history')) {
                          ...
                          if (!tableExists) {
                            console.log(
                              `    Table 'governance_framework_document_history' doesn't exist, clearing ALTER migration record`
                            )
                            shouldClear = true
                          } else {
                ...
                if (stillMissing.length > 0) {
                  console.error(
                    `  ❌ CRITICAL ERROR: ${stillMissing.length} table(s) still missing: ${stillMissing.join(', ')}`
                  )
                  console.error('  Please run: pnpm run reindex:dev')
                  process.exitCode = 1
                } else {
