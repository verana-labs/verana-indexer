...
const PROTECTED_TABLES = ['block', 'block_checkpoint', 'block_signature', 'knex_migrations', 'knex_migrations_lock']

// Also protect any table matching these patterns (block partitions)
const PROTECTED_TABLE_PATTERNS = [/^block_partition_/]
  ...
  try {
    ...
    if (blockCheckpointExists) {
      ...
      if (migrationCheckpointsBackup.length > 0) {
        console.log(
          `   Backed up ${migrationCheckpointsBackup.length} migration checkpoints:`,
          migrationCheckpointsBackup
        )
      }
      ...
      await db('block_checkpoint').whereIn('job_name', genesisJobNames).update({ height: 0 })
      ...
      if (highestBlock > 0) {
        ...
        const crawlBlockUpdated = await db('block_checkpoint').where('job_name', 'crawl:block').update({ height: 0 })
        if (crawlBlockUpdated === 0) {
          await db('block_checkpoint').insert({
            job_name: 'crawl:block',
            height: 0,
...
async function restoreMigrationCheckpoints(db: Knex): Promise<void> {
  ...
  try {
    ...
    for (const checkpoint of migrationCheckpointsBackup) {
      try {
        await db('block_checkpoint').insert(checkpoint).onConflict('job_name').merge()
  ...
  try {
    ...
    for (const [migrationName, tables] of Object.entries(MIGRATION_TO_TABLES)) {
      ...
      if (hasMissingTable) {
        ...
        console.log(
          `  → Migration "${migrationName}" needed (creates: ${tables.filter((t) => missingTables.includes(t)).join(', ')})`
        )
      }
    }
    ...
    const transactionPartitionMigrationNames = ['partition-transaction-table', 'transaction_message_partition']

    const needsTransactionTables =
      missingTables.includes('transaction') || missingTables.includes('transaction_message')
    ...
    if (transactionPartitionMigrationsToClear.length > 0) {
      console.log(
        `  Removing ${transactionPartitionMigrationsToClear.length} transaction partition migration record(s)...`
      )
      for (const migName of transactionPartitionMigrationsToClear) {
        try {
          const deleted = await db('knex_migrations').where('name', migName).delete()
          if (deleted > 0) {
    ...
    for (const alterMig of ALTER_MIGRATIONS) {
      for (const recordedName of allMigrationNames) {
        if (recordedName.includes(alterMig)) {
          ...
          if (shouldClear) {
            try {
              const deleted = await db('knex_migrations').where('name', recordedName).delete()
              if (deleted > 0) {
    ...
    try {
      ...
      if (tablesAfterMigration.length > 0) {
        console.warn(
          `   Warning: ${tablesAfterMigration.length} tables still missing after migrate.latest(): ${tablesAfterMigration.join(', ')}`
        )
      ...
      if (hasParticipantsColumn && hasParticipantsEcosystemColumn) {
        console.log(
          '   ✓ New participant attributes (participants + role counters + slash stats) verified in participants table'
        )
      } else {
      ...
      if (hasEcosystemParticipantsColumn) {
        console.log(
          '   ✓ New trust registry statistics attributes (participants, active_schemas, weight, etc.) verified in ecosystem table'
        )
      } else {
      ...
      if (err.message?.includes('already exists') && err.message.includes('block')) {
        ...
        err.message?.includes('Tables still missing')
      ) {
        ...
        const moduleMigrationsToClear = completed.filter((m: Migration) => {
          ...
          const matchedEntry = Object.entries(MIGRATION_TO_TABLES).find(([migrationName]) =>
        ...
        for (const migration of moduleMigrationsToClear) {
          try {
            const deleted = await db('knex_migrations').where('name', migration.name).delete()
            if (deleted > 0) {
        ...
        try {
          ...
          if (tablesAfterRetry.length > 0) {
            console.error(
              `   ❌ CRITICAL: ${tablesAfterRetry.length} tables still missing: ${tablesAfterRetry.join(', ')}`
            )
    ...
    if (finalMissingTables.length > 0) {
      console.error(
        `  ❌ CRITICAL ERROR: ${finalMissingTables.length} tables still missing after migrations: ${finalMissingTables.join(', ')}`
      )
      ...
      try {
        ...
        if (stillMissing.length > 0) {
          ...
          throw new Error(
            `Critical: ${stillMissing.length} tables still missing after migration retry: ${stillMissing.join(', ')}`
          )
        } else {
        ...
        console.error(`  ❌ Migration retry failed: ${retryErr.message}`)
        throw new Error(
          `Critical: ${finalMissingTables.length} tables still missing after migrations: ${finalMissingTables.join(', ')}`
        )
      }
    } else {
    ...
    if (participantsTableExists) {
      ...
      if (hasParticipants && hasEcosystemSlashEvents && hasNetworkSlashEvents) {
        console.log(
          '  ✓ Participant table has all new attributes (participants, ecosystem_slash_events, network_slash_events, etc.)'
        )
      } else {
    ...
    if (ecosystemTableExists) {
      ...
      if (hasParticipants && hasActiveSchemas && hasWeight && hasEcosystemSlashEvents && hasNetworkSlashEvents) {
        console.log(
          '  ✓ Trust registry table has all new statistics attributes (participants, active_schemas, archived_schemas, weight, issued, verified, slash stats, etc.)'
        )
      } else {
    ...
    if (credentialSchemaTableExists) {
      ...
      if (hasParticipants && hasWeight && hasEcosystemSlashEvents && hasNetworkSlashEvents) {
        console.log(
          '  ✓ Credential schema table has all new statistics attributes (participants, weight, issued, verified, slash stats, etc.)'
        )
      } else {
    ...
    if (err.message?.includes('already exists') && err.message.includes('block')) {
      ...
      if (migrationsToRun.length > 0) {
        ...
        for (const migration of migrationsToRun) {
          try {
            const existing = await db('knex_migrations').where('name', migration.name).first()
            ...
            if (migrationFile.up) {
              ...
              const nextBatch =
                maxBatch && (maxBatch as { max: number | null }).max ? (maxBatch as { max: number }).max + 1 : 1

              const stillExists = await db('knex_migrations').where('name', migration.name).first()
...
;(async function reindexModules(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Database: ${connInfo.database || 'unknown'} @ ${connInfo.host || 'unknown'}:${connInfo.port || 'unknown'}\n`
    )
