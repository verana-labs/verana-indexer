...
function runReindex(checkpoint: ReindexCheckpoint | null): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn('node', [...nodeOptionsParts, scriptPath], {
      stdio: 'inherit',
      shell: true,
      env: {
        ...process.env,
    ...
    child.on('exit', (code, signal) => {
      ...
      if (code === 0) {
        ...
        const errorType =
          code === 134 ? 'JavaScript heap out of memory' : code === null ? 'Unknown error' : `Exit code ${code}`
        ...
        if (attemptCount <= MAX_RESTARTS) {
          ...
          setTimeout(() => {
            runReindex(newCheckpoint).then(resolve).catch(reject)
          }, RESTART_DELAY)
        } else {
    ...
    new Promise((resolve, reject) => {
      ...
      svcProcess.on('exit', (code, signal) => {
        ...
        if (MEMORY_EXIT_CODES.has(exitCode) && restartCount <= MAX_SERVICE_RESTARTS) {
          log(
            `⚠️  Services exited with code ${exitCode} (memory). Restarting in ${SERVICE_RESTART_DELAY / 1000}s... (${restartCount}/${MAX_SERVICE_RESTARTS})`
          )
...
export { clearCheckpoint, loadCheckpoint, runReindex, saveCheckpoint }
