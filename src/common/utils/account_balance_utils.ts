...
const TD_ACCOUNT_KEYS = ['account', 'owner', 'deposit_account', 'depositAccount', 'creator', 'sender', 'validator']

export function extractAccountAddressesFromContent(content: Record<string, unknown> | null | undefined): string[] {
...
const TD_EVENT_ACCOUNT_KEYS = ['account', 'owner', 'deposit_account', 'creator', 'sender', 'validator']

export function extractAccountAddressesFromEvents(events: TxEventLike[], decodeAttributes?: boolean): string[] {
  const out: string[] = []
...
export function extractAccountAddressesFromTdSources(options: {
  ...
  extractAccountAddressesFromContent(messageContent ?? null).forEach((a) => {
    set.add(a)
  })
  if (events?.length) {
    extractAccountAddressesFromEvents(events, decodeEventAttributes).forEach((a) => {
      set.add(a)
    })
  }
...
export function getLcdBaseUrl(): string {
  const env = (typeof process !== 'undefined' && process.env?.LCD_ENDPOINT?.trim()) || ''
  const base = env || Network?.LCD || ''
  return base.replace(/\/$/, '')
}

export async function fetchAccountBalance(address: string): Promise<BalanceEntry[] | null> {
