...
export async function runWithCrawlLock<T>(fn: () => Promise<T>, logger?: any): Promise<T> {
  if (isReindexMode()) {
...
export async function throttleDbPoolIfNeeded(logger?: any): Promise<boolean> {
  ...
  while (waited < settings.maxWaitMs) {
    ...
    const curPendingOk =
      current.pending <= settings.pendingOkAbs || current.pending / current.max <= settings.pendingOkRatio
    if (curUsageOk && curPendingOk) {
      return true
    }

    if (logger?.warn && waited > 0 && waited % (settings.stepMs * settings.warnEverySteps) === 0) {
      logger.warn(`[DB Pool Guard] Waiting for pool (used=${current.used}/${current.max}, pending=${current.pending})…`)
    }
