import { Event, EventAttribute, Transaction } from '../../models'
...
// tx: tx data
export async function getContractActivities(fromBlock: number, toBlock: number) {
  ...
  const wasmEvents = await Event.query()
    ...
    .whereIn('event.type', [Event.EVENT_TYPE.WASM, Event.EVENT_TYPE.INSTANTIATE])
    .where('event.block_height', '>', fromBlock)
    .andWhere('event.block_height', '<=', toBlock)
    .orderBy('event.id', 'asc')

  wasmEvents.forEach((wasmEvent, index) => {
    const wasmAttribute: { key: string; value: string }[] = wasmEvent.attributes.map((attribute: EventAttribute) => ({
      key: attribute.key,
    ...
    contractActivities.push({
      contractAddress: getAttributeFrom(wasmAttribute, EventAttribute.ATTRIBUTE_KEY._CONTRACT_ADDRESS),
      action,
