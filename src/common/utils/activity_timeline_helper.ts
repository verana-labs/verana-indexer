...
const MSG_TYPE_TO_ACTION: Record<string, string> = {
  ...
  [VeranaParticipantMessageTypes.SetParticipantEffectiveUntil]: 'SetParticipantEffectiveUntil',
  ...
  try {
      ...
      prefixes.length > 0
        ? `AND (${prefixes
            .map((p, idx) => {
              ...
              return idx === 0
                ? `transaction_message.type LIKE '${escapedPrefix}%'`
                : `OR transaction_message.type LIKE '${escapedPrefix}%'`
            })
            .join(' ')})`
        : ''
    ...
    let historyQuery = knex(historyTable)
      .select(
        ...
        entityIdField
          ? knex.raw(
              `COALESCE(CAST(${historyTable}.${entityIdField} AS TEXT), CAST(${historyTable}.id AS TEXT)) as activity_entity_id`
            )
    ...
    if (transactionTimestampOlderThan) {
      ...
      if (!Number.isFinite(cutoffHeight)) {
        ...
        effectiveMaxHeight = effectiveMaxHeight == null ? cutoffHeight : Math.min(effectiveMaxHeight, cutoffHeight)
      }
    }

    if (effectiveMaxHeight != null) {
      historyQuery = historyQuery.where(function () {
        this.whereNotNull(`${historyTable}.height`).andWhere(
          `${historyTable}.height`,
          '<=',
          effectiveMaxHeight as number
        )
      })
    }
  ...
  try {
    ...
    for (const r of limitedRecords) {
      ...
      if (atype === 'GovernanceFrameworkVersion') {
        const entry = {
          id:
            typeof r.activity_entity_id === 'number'
              ? r.activity_entity_id
              : typeof r.id === 'number'
                ? r.id
                : Number(r.id || r.activity_entity_id || 0),
          version: typeof r.version === 'number' ? r.version : Number(r.version || 0),
          active_since: r.active_since
            ? r.active_since instanceof Date
              ? r.active_since.toISOString()
              : new Date(r.active_since).toISOString()
            : null,
          created: r.created
            ? r.created instanceof Date
    ...
    return limitedRecords.map((record: any) => {
      ...
      if (changes && Object.hasOwn(changes, 'height')) {
      ...
      if (changes) {
        if (activityEntityType === 'TrustDeposit') {
          ...
          if (Object.hasOwn(changes, 'json_schema') && changes.json_schema != null) {
