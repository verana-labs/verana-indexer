function parseMultiplier(rawValue: string | undefined, fallback: number, maxValue: number = 100): number {
...
const NORMAL_SPEED_MULTIPLIER = parseMultiplier(process.env.CRAWL_SPEED_MULTIPLIER, 5, 20)

const REINDEX_SPEED_MULTIPLIER = parseMultiplier(process.env.CRAWL_SPEED_MULTIPLIER_REINDEX, 80, 1000)
...
export function applySpeedToDelay(baseDelay: number, isReindexing: boolean = false): number {
  ...
  if (isReindexing) {
    return Math.max(REINDEX_MIN_DELAY_MS, Math.floor(safeBaseDelay / REINDEX_EFFECTIVE_MULTIPLIER))
  }

  return Math.max(FRESH_MIN_DELAY_MS, Math.floor(safeBaseDelay / FRESH_EFFECTIVE_MULTIPLIER))
}
...
export function applySpeedToBatchSize(baseBatchSize: number, isReindexing: boolean = false): number {
  ...
  if (isReindexing) {
    return Math.min(REINDEX_MAX_BATCH_SIZE, Math.max(5, safeBaseBatchSize * REINDEX_EFFECTIVE_MULTIPLIER))
  }

  return Math.min(FRESH_MAX_BATCH_SIZE, Math.max(5, safeBaseBatchSize * FRESH_EFFECTIVE_MULTIPLIER))
}

export function getRecommendedConcurrency(baseConcurrency: number = 5, isReindexing: boolean = false): number {
  const safeBaseConcurrency = toPositiveInt(baseConcurrency, 1)

  if (isReindexing) {
    return Math.min(REINDEX_MAX_CONCURRENCY, Math.max(2, safeBaseConcurrency * REINDEX_EFFECTIVE_MULTIPLIER))
  }

  return Math.min(FRESH_MAX_CONCURRENCY, Math.max(2, safeBaseConcurrency * FRESH_EFFECTIVE_MULTIPLIER))
}
