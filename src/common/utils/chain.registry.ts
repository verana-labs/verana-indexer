...
export default class ChainRegistry {
  ...
  constructor(logger: LoggerInstance, providerRegistry: IProviderRegistry) {
    ...
    const txRegistryTypes = Array.isArray(this.txRegistryType)
      ? (this.txRegistryType as string[]).map((type: string) => [type, _.get(this, type.slice(1))])
      : []
    this.registry = new Registry([...defaultStargateTypes, ...wasmTypes, ...veranaRegistry, ...txRegistryTypes] as any)
  }
  ...
  public decodeMsg(msg: unknown): unknown {
    ...
    if (typedMsg && typeof typedMsg === 'object' && 'typeUrl' in typedMsg && typeof typedMsg.typeUrl === 'string') {
      ...
      const msgType = this.registry.lookupType(typedMsg.typeUrl) as TsProtoGeneratedType
      this._logger.warn('msgType', msgType)
      if (!msgType) {
        const formattedValue = typedMsg.value instanceof Uint8Array ? toBase64(typedMsg.value) : typedMsg.value
        result.value = formattedValue

        const isVeranaMessage = typedMsg.typeUrl.startsWith('/verana.')
        const isCosmosSystemMessage =
          typedMsg.typeUrl.includes('.upgrade.') ||
        ...
        try {
          ...
          this._logger.error('Failed to decode message; returning raw value', typedMsg.typeUrl, err?.message ?? err)
      ...
      if (
        ...
        try {
          ...
          if (packet?.data && typeof packet.data === 'string') {
            result.packet = {
              ...packet,
              data: JSON.parse(fromUtf8(fromBase64(packet.data))),
            }
          }
          if (acknowledgement && typeof acknowledgement === 'string') {
            result.acknowledgement = JSON.parse(fromUtf8(fromBase64(acknowledgement)))
          }
        } catch (_error) {
          ...
          if (Array.isArray(msgs)) {
            result.msgs = msgs.map((subMsg) => {
              ...
              return this.decodeMsg({
                ...
                value: Utils.isBase64(valueStr) ? fromBase64(valueStr) : subMsg.value,
              })
            })
          }
        } catch (_error) {
