...
console.log(
  `[DB Connection] Initialized with pool size: ${poolMax} (env: ${environment}, POSTGRES_POOL_MAX: ${postgresPoolMax || 'not set'})`
)
...
if (process.env.NODE_ENV !== 'test') {
  ...
  const runMemoryGuard = () => {
    ...
    if (
      heapUsed >= GC_TRIGGER_HEAP_BYTES &&
      heapUsed < MEMORY_CRITICAL_HEAP_BYTES &&
      now - lastGcTriggerAt >= GC_TRIGGER_INTERVAL_MS
    ) {
      lastGcTriggerAt = now
      ...
      ;(async () => {
        try {
          ...
          if (!shouldPauseCrawlerForMemory()) {
            if (logger?.warn) {
              logger.warn(
                `[Memory] CRITICAL heap detected during reindexing; skipping automatic crawl pause (heap ${(heapUsed / 1024 / 1024).toFixed(0)} MB).`
              )
            } else {
          ...
          if (logger?.warn) {
            logger.warn(
              `[Memory] CRITICAL: pausing crawler to avoid OOM (heap ${(heapUsed / 1024 / 1024).toFixed(0)} MB). Will auto-resume when memory recovers.`
            )
          } else {
  ...
  poolStatusInterval = setInterval(() => {
    ...
    if (knex.client && (knex.client as any).pool) {
      ...
      if ((usedCount >= warningThreshold || pendingCount > 10) && now - lastWarningTime > WARNING_INTERVAL) {
        const logger = (global as any).logger
        ...
        if (now - lastCriticalAt >= CRITICAL_ACTION_THROTTLE_MS) {
          ...
          ;(async () => {
            try {
              ...
              if (logger?.warn) {
                logger.warn(
                  `[DB Pool] CRITICAL: pausing crawler until pool drains (pending=${pendingCount}, used=${usedCount}/${poolMax}). Will auto-resume when pool recovers.`
                )
              } else {
                console.warn(
                  `[DB Pool] CRITICAL: pausing crawler until pool drains. Will auto-resume when pool recovers.`
                )
              }
            } catch (err) {
