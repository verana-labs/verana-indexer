...
export default class QueueManager {
   ...
   */
  public async registerQueueHandler(qOpt: QueueOptions, fn: (payload: object) => Promise<void>) {
   ...
   * @returns
   */
  public async createJob(queueName: string, jobName?: string, opts?: JobsOptions, payload?: object): Promise<void> {
  ...
  private sanitizeJobOptions(opts?: JobsOptions, queueName?: string, jobName?: string): JobsOptions | undefined {
    if (!opts) return opts
    ...
    if (repeatNormalized.every !== undefined && !Number.isFinite(repeatNormalized.every)) {
      repeatNormalized.every = undefined
    }
    ...
    const sanitizeCronLikeField = (field: 'pattern' | 'cron') => {
      ...
      if (trimmed.length === 0 || /^(undefined|null|nan)$/i.test(trimmed) || /\bundefined\b/i.test(trimmed)) {
    ...
    if (
      ...
      ;(normalized as Record<string, unknown>).repeat = undefined
